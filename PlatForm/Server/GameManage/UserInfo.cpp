...
//更改用户分数
bool CGameUserInfo::ChangePoint(__int64 dwPoint, __int64 dwTaxCom,bool bWin, bool bLost, bool bMid, bool bCut, long int dwPlayCount, __int64 &dwMoney, bool bBuyDesk/* = false*/)
{
	if (m_bAccess)
	{
		CString stime;
		...
		if(m_UserData.i64Money<=0 && !bBuyDesk)
		{
...
int CGameUserInfo::GetDistance(float flat,float flnt)
{
	double radLat1 = rad(m_UserData.flat);
	double radLat2 = rad(flat);
	double a = radLat1 - radLat2;
	double b = rad(m_UserData.flnt) - rad(flnt);
...
CGameUserInfo * CGameUserInfoManage::RegNetCutUser(CGameUserInfo * pNetCutUserInfo)
{
	...
	if (pRegUserInfo!=NULL)
	{
		...
		//pRegUserInfo->bBeforeState = pNetCutUserInfo->m_UserData.bUserState;
		if (pRegUserInfo->m_UserData.bUserState == USER_ARGEE)
			pRegUserInfo->bBeforeState = USER_SITTING;
		else
			pRegUserInfo->bBeforeState = pRegUserInfo->m_UserData.bUserState;
