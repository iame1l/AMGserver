...
void ContestPrintf(const char *p, ...)
{
	...
	wsprintf( szFilename, TEXT("Log\\%s_%s_contest.txt"), szDateStr, c_szDebugFileName );
	va_list arg;
	va_start( arg, p );
	FILE *fp = NULL;
...
void ErrorPrintf(const char *p, ...)
{
	...
	GetTimeFormatA( LOCALE_USER_DEFAULT, TIME_FORCE24HOURFORMAT, &sysTime, "HH:mm:ss", szTimeStr, 32);
	GetDateFormatA( LOCALE_USER_DEFAULT, LOCALE_USE_CP_ACP, &sysTime, "yyyy-MM-dd", szDateStr, 32);
	wsprintf( szFilename, TEXT("Log\\%s_%sError.txt"),szDateStr, c_szDebugFileName );
	va_list arg;
	va_start( arg, p );
	FILE *fp = NULL;
	fp = fopen(szFilename, "a" );
...
void CAutoAllotDesk::AllotDesk(int& iDeskCount, int* &arrDesk,bool bContestRule)
{
	...
	{
		if (!bContestRule && m_UserList[_iBegin].m_pGameUserInfo->m_UserData.isVirtual)
		{//不是比赛场并且桌子第一个人就是机器人
			break;
		}
