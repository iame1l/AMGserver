...
unsigned char g_chSecretKey[128];
CString g_StrSecretKey;


void GeneratorSecretKey()
{
	static bool bGenerator = false;
	if (!bGenerator)
	{
		char strSecretKey[10];
		int nLen = sprintf((char*)strSecretKey, "%d", SECRET_KEY);

		unsigned char szMDTemp[17];
		char szMDKey[33];
		MD5_CTX Md5;
...
void DebugPrintfSocket(const char *p, ...)
{
	char szFilename[256];
	sprintf_s( szFilename, "DebugInfo\\SocketMessage.txt");
	FILE *fp;
	...
	va_list arg;
	va_start( arg, p );
	fprintf_s(fp,"[%s]--", CTime::GetCurrentTime().Format("%H:%M:%S"));
	vfprintf(fp, p, arg );
	fprintf(fp,"\n");
	fclose(fp);
}
...
int CTCPClientSocket::SendData(void * pData, UINT uSize, BYTE bMainID, BYTE bAssistantID, BYTE bHandleCode)
{
	if ((m_hSocket!=INVALID_SOCKET)&&(uSize<=MAX_SEND_SIZE))
	{
		//定义数据
		int iErrorCode=0;
		char bSendBuffer[MAX_SEND_SIZE_OUT+NET_HEAD_SIZE] = {0};
        ...
        if(uSize > 0)
        {
            ...
            memcpy(bSendBuffer + NET_HEAD_SIZE, pDataCopy, pNetHead->uMessageSize-NET_HEAD_SIZE); 
		...
		do
		{
			iErrorCode=::send(m_hSocket,bSendBuffer+uSended,pNetHead->uMessageSize-uSended,0);
			...
			uSended+=iErrorCode;
		} while (uSended<pNetHead->uMessageSize);
...
ULONG CTCPSocketManage::IPToULong(TCHAR * szIP)
{
    char* p;  
    int sections[4]={0};  
    int i=0;  
...
ULONG CTCPSocketManage::IPToULongEx(char* szIP)
{
	char* p = NULL;  
	int sections[4]={0};  
	int i=0;  
	...
	while (pos != -1)
	{
		sections[i] = atoi(strIP.substr(0,pos).c_str()); 
...
int CTCPSocketForZ::SendData(void * pData, UINT uBufLen, BYTE bMainID, BYTE bAssistantID, BYTE bHandleCode, DWORD dwHandleID)
{
	if (uBufLen<=MAX_SEND_SIZE)
	{
        ...
        int nNetMessageHeadLen = sizeof(NetMessageHead);
        unsigned int uSize = pNetHead->uMessageSize - nNetMessageHeadLen;
        if (uSize > 0 && NULL != pData)
        {
            HNRC4 RC4;
            RC4.init(g_chSecretKey,  g_StrSecretKey.GetLength());
...
bool CTCPSocketForZ::OnRecvCompleted(DWORD dwHandleID)
{
	...
	try
	{
		...
		{
			//处理数据
			//DebugPrintfSocket("Recive Data %d from %d",iRecvCode,m_uIndex);
			m_dwRecvBuffLen+=iRecvCode;
			NetMessageHead * pNetHead=(NetMessageHead *)m_szRecvBuf;
			if(pNetHead->bMainID==200)
			{
...
{
	if ((uIndex < m_SocketPtrArray.GetPermitCount()) && m_bRun)
	{
		return m_SocketPtrArray.GetArrayItem(uIndex)->SendData(bMainID, bAssistantID, bHandleCode, dwHandleID);
	}
	return 0;
...
{
	// 效验数据
	if (!m_bRun || (uSize > (sizeof(m_SendLine[0].szSendData) - sizeof(NetMessageHead))))
	{
		return 0;
	}

	CSignedLockObject BatchLock(&m_csBacthSend,true);
	if (m_uBatchLineCount >= uBatchLineSize)
	{
		return 0;
	}
	...
	if (uSize > 0)
	{
		CopyMemory(m_SendLine[m_uWritePos].szSendData,pData,uSize);
	}
...
// 组播发送函数
int CTCPSocketManageForZ::SendDataUnion(UINT uUnionID, void * pData, UINT uSize, BYTE bMainID, BYTE bAssistantID, BYTE bHandleCode)
{
	// 效验数据
...
//添加用户到群组
int CTCPSocketManageForZ::UnionAddUser(UINT uUnionID, UINT uSocketID)
{
...
//从群组删除用户
int CTCPSocketManageForZ::UnionRemoveUser(UINT uUnionID, UINT uSocketID)
{
	map<int,vector<int>>::iterator l_it;
	l_it = m_UnionData.find(uUnionID);
	if (l_it != m_UnionData.end())
...
//移除整个群组
int CTCPSocketManageForZ::UnionRemoveAll(UINT uUnionID)
{
	map<int,vector<int>>::iterator l_it;
	l_it = m_UnionData.find(uUnionID);
	if (l_it != m_UnionData.end())
...
//用户下线移除用户所有群组
int CTCPSocketManageForZ::UnionRemovUserAll(UINT uSocketID)
{
...
//SOCKET 组播发送数据线程
unsigned __stdcall CTCPSocketManageForZ::ThreadUnionSend(LPVOID pThreadData)
{
	//数据定义
	ThreadStartStruct		* pStartData=(ThreadStartStruct *)pThreadData;					//线程启动数据指针
	CTCPSocketManageForZ	* pSocketManage=pStartData->pSocketManage;						//管理指针
