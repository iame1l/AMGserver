...
bool CBaseMainManageForZ::Init(ManageInfoStruct * pInitData, IDataBaseHandleService * pDataHandleService)
{
	...
	{
		throw new CAFCException(TEXT("CBaseMainManageForZ::Init PreInitParameter 参数调节错误"),0x41A);
	}
	...
	// 调用接口
	if (!OnInit(&m_InitData,&m_KernelData))
	{
...
{	
	...
	if (StartEvent == NULL)
	{
		throw new CAFCException(TEXT("CBaseMainManageForZ::Start 事件建立失败"),0x41C);
	}
	...
	{
		throw new CAFCException(TEXT("CBaseMainManageForZ::Start m_hCompletePort 建立失败"),0x41D);
	}
	...
	{
		throw new CAFCException(TEXT("CBaseMainManageForZ::Start LineDataHandleThread 线程启动失败"),0x41E);
	}
	...
	if (m_KernelData.bStartSQLDataBase)
	{
		m_SQLDataManage.Start();
	}
	
	if (m_KernelData.bStartTCPSocket)
	{
		m_TCPSocket.Start(m_KernelData.uAcceptThreadCount,m_KernelData.uSocketThreadCount);
	}
	
	// 调用接口
	if (!OnStart())
	{
		throw new CAFCException(TEXT("CBaseMainManageForZ::Start OnStart 函数错误"),0x41F);
	}
...
unsigned __stdcall CBaseMainManageForZ::LineDataHandleThread(LPVOID pThreadData)
{
	...
	while (1)
	{
		...
		while(m_pDataLine->GetDataCount())//当队列中处理信息过多,会不会占用太多的cpu时间而使其他线程延时严重
		{
			...
			{
				// 处理完成端口数据
				if (!bRun || (m_pDataLine->GetData(pDataLineHead,sizeof(szBuffer)) == 0))
				{
					continue;
				}
				...
				case HD_SOCKET_READ:		//SOCKET 数据读取
					{
						...
						try
						{
							//DebugPrintfZ("OnSocketRead %d %d",pSocketRead->NetMessageHead.bMainID,pSocketRead->NetMessageHead.bAssistantID);
				...
				case HD_TIMER_MESSAGE:		//定时器消息
					{
						...
						HNLOG_AFC("HD_TIMER_MESSAGE %d",pTimerMessage->uTimerID);
