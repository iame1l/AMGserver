...
bool CBaseMainManage::Init(ManageInfoStruct * pInitData, IDataBaseHandleService * pDataHandleService)
{
	...
	if (!PreInitParameter(&m_InitData,&m_KernelData)) 
		throw new CAFCException(TEXT("CBaseMainManage::Init PreInitParameter 参数调节错误"),0x41A);
	...
	if (OnInit(&m_InitData,&m_KernelData)==false)  
		throw new CAFCException(TEXT("CBaseMainManage::Init OnInit 函数错误"),0x41B);
...
bool CBaseMainManage::Start()
{
	...
	if (StartEvent==NULL) throw new CAFCException(TEXT("CBaseMainManage::Start 事件建立失败"),0x41C);

	//建立完成端口
	m_hCompletePort = ::CreateIoCompletionPort(INVALID_HANDLE_VALUE,NULL,NULL,0);
	if (m_hCompletePort == NULL) throw new CAFCException(TEXT("CBaseMainManage::Start m_hCompletePort 建立失败"),0x41D);
	...
	if (m_hHandleThread == NULL) throw new CAFCException(TEXT("CBaseMainManage::Start LineDataHandleThread 线程启动失败"),0x41E);
	...
	//调用接口
	if (!OnStart()) throw new CAFCException(TEXT("CBaseMainManage::Start OnStart 函数错误"),0x41F);
...
//网络关闭处理
bool CBaseMainManage::OnSocketCloseEvent(ULONG uAccessIP, UINT uIndex, long int lConnectTime)
{
...
//数据库结果处理
bool CBaseMainManage::OnDataBaseResultEvent(DataBaseResultLine * pResultData, UINT uHandleKind, UINT uHandleResult, UINT uResultSize, UINT uIndex, DWORD dwHandleID)
{
	//包装数据
...
bool CBaseMainManage::AFCKernelStart()
{
	...
	//建立事件
	CEvent StartEvent(FALSE,TRUE,NULL,NULL);
	if (StartEvent==NULL) throw new CAFCException(TEXT("CBaseMainManage::AFCKernelStart 事件建立失败"),0x420);
	...
	if (m_hWindowThread==NULL) throw new CAFCException(TEXT("CBaseMainManage::AFCKernelStart WindowMsgThread 线程建立失败"),0x421);
	::WaitForSingleObject(ThreadData.hEvent,INFINITE);
	if (ThreadData.bSuccess==FALSE) throw new CAFCException(TEXT("CBaseMainManage::AFCKernelStart WindowMsgThread 线程建立失败"),0x422);
