...
struct WindowThreadStartStruct
{
	...
	CBaseMainManageForWeb						* pMainManage;				//数据管理指针
};
...
//构造函数
CBaseMainManageForWeb::CBaseMainManageForWeb(void)
{
	m_bInit=false;
	m_bRun=false;
	m_hWindow=NULL;
...
//初始化函数 
bool CBaseMainManageForWeb::Init(ManageInfoStruct * pInitData, IDataBaseHandleService * pDataHandleService)
{
	if ((this == NULL) || m_bInit) 
		return false;
	...
	this->m_InitData;
	if (!PreInitParameter(&m_InitData,&m_KernelData)) 
		throw new CAFCException(TEXT("CBaseMainManageForWeb::Init PreInitParameter 参数调节错误"),0x41A);
	...
	if (OnInit(&m_InitData,&m_KernelData)==false)  
		throw new CAFCException(TEXT("CBaseMainManageForWeb::Init OnInit 函数错误"),0x41B);
...
//取消初始化函数 
bool CBaseMainManageForWeb::UnInit()
{
...
//启动函数
bool CBaseMainManageForWeb::Start()
{
	if ((this == NULL) || m_bRun || !m_bInit) return false;

	//建立事件
	CEvent StartEvent(FALSE,TRUE,NULL,NULL);
	if (StartEvent==NULL) throw new CAFCException(TEXT("CBaseMainManageForWeb::Start 事件建立失败"),0x41C);

	//建立完成端口
	m_hCompletePort = ::CreateIoCompletionPort(INVALID_HANDLE_VALUE,NULL,NULL,0);
	if (m_hCompletePort == NULL) throw new CAFCException(TEXT("CBaseMainManageForWeb::Start m_hCompletePort 建立失败"),0x41D);
	...
	if (m_hHandleThread == NULL) throw new CAFCException(TEXT("CBaseMainManageForWeb::Start LineDataHandleThread 线程启动失败"),0x41E);
	...
	//调用接口
	if (!OnStart()) throw new CAFCException(TEXT("CBaseMainManageForWeb::Start OnStart 函数错误"),0x41F);
...
//停止服务
bool CBaseMainManageForWeb::Stop()
{
	if (this == NULL) return false;
	...
	//关闭窗口
	if ((m_hWindow != NULL) && ::IsWindow(m_hWindow)) ::SendMessage(m_hWindow,WM_CLOSE,0,0);
...
//网络关闭处理
bool CBaseMainManageForWeb::OnSocketCloseEvent(ULONG uAccessIP, UINT uIndex, long int lConnectTime)
{
...
//网络消息处理
bool CBaseMainManageForWeb::OnSocketReadEvent(CTCPSocketForWeb * pSocket, NetMessageHead * pNetHead, void * pData, UINT uSize, UINT uIndex, DWORD dwHandleID)
{
...
//定时器通知消息
bool CBaseMainManageForWeb::WindowTimerMessage(UINT uTimerID)
{
...
//数据库结果处理
bool CBaseMainManageForWeb::OnDataBaseResultEvent(DataBaseResultLine * pResultData, UINT uHandleKind, UINT uHandleResult, UINT uResultSize, UINT uIndex, DWORD dwHandleID)
{
	//包装数据
...
//设定定时器
bool CBaseMainManageForWeb::SetTimer(UINT uTimerID, UINT uElapse)
{
...
//清除定时器
bool CBaseMainManageForWeb::KillTimer(UINT uTimerID)
{
	if ((m_hWindow != NULL) && ::IsWindow(m_hWindow))
	{
...
//内核初始化函数
bool CBaseMainManageForWeb::AFCKernelStart()
{
	if ((m_hWindow!=NULL)&&(::IsWindow(m_hWindow)==TRUE)) return false;

	//建立事件
	CEvent StartEvent(FALSE,TRUE,NULL,NULL);
	if (StartEvent==NULL) throw new CAFCException(TEXT("CBaseMainManageForWeb::AFCKernelStart 事件建立失败"),0x420);
	...
	if (m_hWindowThread==NULL) throw new CAFCException(TEXT("CBaseMainManageForWeb::AFCKernelStart WindowMsgThread 线程建立失败"),0x421);
	::WaitForSingleObject(ThreadData.hEvent,INFINITE);
	if (ThreadData.bSuccess==FALSE) throw new CAFCException(TEXT("CBaseMainManageForWeb::AFCKernelStart WindowMsgThread 线程建立失败"),0x422);

	return true;
}

//队列数据处理线程
unsigned __stdcall CBaseMainManageForWeb::LineDataHandleThread(LPVOID pThreadData)
{
	//数据定义
	HandleThreadStartStruct		* pData=(HandleThreadStartStruct *)pThreadData;		//线程启动数据指针
	CBaseMainManageForWeb				* pMainManage=pData->pMainManage;					//数据管理指针
	...
	while (1)
	{
		...
		//处理数据
		while(m_pDataLine->GetDataCount())//[问题]当队列中处理信息过多,会不会占用太多的cpu时间而使其他线程延时严重
		{
			//处理数据
			try
			{
...
//WINDOW 消息循环线程
unsigned __stdcall CBaseMainManageForWeb::WindowMsgThread(LPVOID pThreadData)
{
...
//窗口回调函数
LRESULT CALLBACK CBaseMainManageForWeb::WindowProcFunc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
	case WM_CREATE:		//窗口建立消息
		{
			...
			CBaseMainManageForWeb * pMainManage=(CBaseMainManageForWeb *)(((CREATESTRUCT *)lParam)->lpCreateParams);
	...
	case WM_TIMER:		//定时器消息
		{
			...
			CBaseMainManageForWeb * pMainManage=(CBaseMainManageForWeb *)::TlsGetValue(iIndex);
