...
{
	//父管理模块
	m_pMainManage = (CGameLogonManage*)pWnd;
...
bool CBankService4Z::OnNetMessage(NetMessageHead *pNetHead, void* pData, UINT uSize, ULONG uAccessIP, UINT uIndex, DWORD dwHandleID)
{
	...
	if (pNetHead->bMainID != MDM_GP_BANK && pNetHead->bMainID != MDM_GP_MONEY)
	{
		return true;
	}
	...
	else if (pNetHead->bMainID == MDM_GP_MONEY)
	{
		return notifyMoneyTransSuccess(pData,uSize,uIndex,dwHandleID);
	}
...
bool CBankService4Z::OnTransferReponse(DataBaseResultLine* pResultData)
{
	...
	if (HC_GP_BANK_TRANSFER_SUC == pResultData->uHandleRusult)
	{
		...
		if (nTargetIdx != -1)
		{
			m_pLogonManage->m_TCPSocket.SendData(nTargetIdx,&_Result,sizeof(TMSG_GP_BankTransfer),MDM_GP_NOTICE_MSG,ASS_GR_TRANSFER_MONEY, ASS_GR_TRANSFER_SUC,0);
		}
		...
		if(pUser && pUser->iVipTime>curtime)
		{
			DL_GP_I_UserRebateSetStruct cmd;
			cmd.bTag = 0;
...
bool CBankService4Z::OnTransRecordResponse(DataBaseResultLine* pResultData)
{
	...
	if (0 == pTransRecord->iCount)
	{
		//return true;
	}

	//时间类型转换
	for (int i=0;i<pTransRecord->iCount;i++)
	{
		SYSTEMTIME systime;
		VariantTimeToSystemTime(pTransRecord->datatime[i], &systime);
		CTime timeTmp(systime);
		pTransRecord->_data[i].oleDateTime = timeTmp.GetTime();
	}

	char bBuffer[MAX_SEND_SIZE];
	memset(bBuffer,0,sizeof(bBuffer));
	int iMax = (MAX_SEND_SIZE-24)/sizeof(TMSG_GP_BankTranRecord);
	int SendCount = 0;
	int SendTotalcount = 0;

	while(SendCount*iMax < pTransRecord->iCount)
	{
		int PreSend = (pTransRecord->iCount-SendCount*iMax)>iMax?iMax:(pTransRecord->iCount-SendCount*iMax);
		for (int i = 0;i<PreSend;i++)
		{
			memcpy_s(bBuffer+i*sizeof(TMSG_GP_BankTranRecord),sizeof(TMSG_GP_BankTranRecord),&pTransRecord->_data[i+SendCount*iMax],sizeof(TMSG_GP_BankTranRecord));
		}
		m_pLogonManage->m_TCPSocket.SendData(pResultData->uIndex, bBuffer, sizeof(TMSG_GP_BankTranRecord)*PreSend,MDM_GP_BANK,ASS_GP_BANK_TRANS_RECORD, 0, 0);
		SendCount++;
	}

	m_pLogonManage->m_TCPSocket.SendData(pResultData->uIndex, MDM_GP_BANK,ASS_GP_BANK_TRANS_RECORD, 1, 0);

	SafeDeleteArray(pTransRecord->_data);
...
bool CBankService4Z::notifyMoneyTransSuccess(void* pData,UINT uSize,UINT uIndex,DWORD dwHandleID)
{
	if (sizeof(MSG_GR_S_TransferMoney) == uSize) 
	{
		MSG_GR_S_TransferMoney * pTransfer = (MSG_GR_S_TransferMoney *)pData;		
		bool bRet =	m_pLogonManage->IsUserAlreadyLogon(pTransfer->destUserID);
		int iSocket = 0;
		if (bRet)
		{
			iSocket = m_pLogonManage->GetIndexByID(pTransfer->destUserID);	
			if (iSocket <= 0)
				return true;
			return m_pLogonManage->m_TCPSocket.SendData(iSocket, pData, uSize, MDM_GP_NOTICE_MSG, ASS_GR_TRANSFER_MONEY, ASS_GR_TRANSFER_SUC, 0); //发送提示消息给被转账人。
		}
		return true;
	}
}