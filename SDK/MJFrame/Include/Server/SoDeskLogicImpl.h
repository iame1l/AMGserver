...
public:
	...
	///旁观参数变化
	virtual bool OnWatchSetChange(BYTE bDeskStation, long int dwTargetUserID, bool bEnableWatch);
	...
	//更新奖池数据(游戏->平台)
	/// @return 机器人赢钱数
	virtual void UpDataRoomPond(__int64 iAIHaveWinMoney){return ;};
...
public:
	...
	*	@brief 发送旁观消息封装
	*/
	int SendDeskWatchGameDataByID(UINT nUserID, UINT bAssID, BYTE *pBuffer, int nLen);
...
private:
	...
	*  @brief 玩家设置旁观状态
	*/
	int UserSetWatch(int nDeskStation, BYTE* pBuffer, int nLen);
};
};
