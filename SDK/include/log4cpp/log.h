...
namespace XAFCLog
{
	...
	static void ClearOldLog()
	{
		...
		for (int i = 0; i < 365; i++)
		{
			...
			sprintf_s(szFile, ".\\log\\%s", s.GetBuffer());
			DeleteDir(szFile);
		}
	}
	...
	static log4cpp::Category* CreateLog(const string& logname, int nametype = 1, log4cpp::Priority::Value priority = LOG_WARN)
	{
		...
		if (1 == nametype)
		{
			...
			sprintf_s(logfile, ".\\log\\%s\\", szDateStr);
			CreateDirectory(logfile, NULL);
			sprintf_s(logfile, ".\\log\\%s\\%s.ini", szDateStr, logname.c_str());
		}
		else
		{
			sprintf_s(logfile, ".\\log\\%s.ini", logname.c_str());
		}
...
// 				return;
			...
			{
				sprintf_s(m_buf, "%s %s %d ", m_filename.c_str(), m_funtionname.c_str(), m_line);
				...
				if (LogType(name) == 1)
				{
					...
					sprintf_s(logfile, ".\\log\\%s\\%s.ini", szDateStr, name.c_str());
