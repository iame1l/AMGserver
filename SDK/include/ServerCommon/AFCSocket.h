...
//缓存定义
#define SED_SIZE				50*1024*5							//缓冲区大小
#define RCV_SIZE				50*1024*5							//缓冲区大小
//缓存定义用于Z服务器，是W的10分之1
#define SED_SIZE_Z				50000*5								//缓冲区大小
#define RCV_SIZE_Z				50000*5								//缓冲区大小

//常量定义
const UINT uBatchLineSize=180;

extern unsigned char g_chSecretKey[128];
extern CString g_StrSecretKey;
...
public:
	...
	//发送数据函数
	inline int CTCPSocket::SendData(const char * pData, UINT uBufLen);
	//发送数据函数
...
public:
	...
	static ULONG IPToULongEx(char* szIP);
	//获取本机 IP 地址
