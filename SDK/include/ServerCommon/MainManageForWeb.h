...
#ifndef MAINMANAGEFORWEB_H_FILE
#define MAINMANAGEFORWEB_H_FILE
...
///基础数据管理类
class AFX_EXT_CLASS CBaseMainManageForWeb : public IServerSocketServiceForWeb , public IDataBaseResultService
{
	///控件变量
protected:
	bool									m_bRun;						///运行标志
...
public:
    ...
    CTCPSocketManageForWeb					m_TCPSocket;				///网络模块
...
public:
	///构造函数
	CBaseMainManageForWeb(void);
	///析构函数
	virtual ~CBaseMainManageForWeb(void);
...
public:
	...
	///网络消息处理 （不需要重载）
	virtual bool OnSocketReadEvent(CTCPSocketForWeb * pSocket, NetMessageHead * pNetHead, void * pData, UINT uSize, UINT uIndex, DWORD dwHandleID);
	///数据库结果处理 （不需要重载）
