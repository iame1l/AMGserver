...
#ifndef CTCPCLIENTWEBSOCKET_HEAD_FILE
#define CTCPCLIENTWEBSOCKET_HEAD_FILE
...
/*namespace AFC
{
	...
	{*/
		//类说明
		class CTCPClientSocketForWeb;

		/*******************************************************************************************************/

		//客户端网络服务接口
		interface  IClientSocketServiceForWeb
		{
			//接口函数 
		public:
			//网络读取消息
			virtual bool OnSocketReadEvent(NetMessageHead * pNetHead, void * pNetData, UINT uDataSize, CTCPClientSocketForWeb * pClientSocket)=0;
			//网络连接消息
			virtual bool OnSocketConnectEvent(UINT uErrorCode, CTCPClientSocketForWeb * pClientSocket)=0;
			//网络关闭消息
		...
		//客户端 SOKET
		class AFX_EXT_CLASS CTCPClientSocketForWeb : public CWnd
		{
			DECLARE_MESSAGE_MAP()
		...
		private:
			...
			IClientSocketServiceForWeb			* m_pIService;					//处理接口
			BYTE								m_szBuffer[RCV_SIZE];			//数据缓存区
            char                                m_szRecvMsg[MAX_SEND_SIZE];
            int                                 m_szRecvMsgPos;
            char                                m_szSendBuffer[MAX_SEND_SIZE + NET_HEAD_SIZE + 14];
			
			int									m_iCheckCode;
			//函数定义
		public:
			//构造函数
			CTCPClientSocketForWeb(IClientSocketServiceForWeb * pIService);
			//析构函数
			virtual ~CTCPClientSocketForWeb();
			//初始化 SOCKET 窗口
			bool InitSocketWnd();
			//是否已经连接
        ...
        protected:
            ...
            int SendDirectData(const void *pData, int len);
...
#endif // CTCPCLIENTWEBSOCKET_HEAD_FILE