...
bool CClientGameDlg::SetGameStation(void * pStationData, UINT uDataSize)
{
	...
	case GS_WAIT_NEXT:			//等待下一盘开始
		{	
			...
			m_iRobNTTime	= pGameStation->iRobNTTime;
			m_iDoubleTime	= pGameStation->iDoubleTime;
			...
			iTime = ((rand()+GetMeUserInfo()->bDeskStation)%(2))+1;
			...
			//所有玩家的状态
			memset(m_iUserStation,STATE_NULL,sizeof(m_iUserStation));
	...
	case GS_ROB_NT:		//抢庄状态
		{
			...
			if (STATE_NULL != m_iUserStation[GetMeUserInfo()->bDeskStation]
				&& STATE_HAVE_ROBNT != m_iUserStation[GetMeUserInfo()->bDeskStation])
			{
				int iTime = 1;
				if (m_iRobNTTime > 10)
				{
					iTime = (rand()%(m_iRobNTTime -10))+1;
				}
				else if (m_iRobNTTime > 0)
				{
					iTime = (rand()%(m_iRobNTTime))+1;
				} else 
				{
					iTime = ((rand()+GetMeUserInfo()->bDeskStation)%(2))+1;
				}
	...
	case GS_DOUBLE:		//下注状态
		{
			GameStation_Double *pGameStation = (GameStation_Double *)pStationData;
			if(NULL == pGameStation)
			{
				return TRUE;
			}
			...
			//庄家位置
			m_iUpGradePeople	 = pGameStation->iStartPos;
			//下注限制
			//m_iThisTurnLimit = pGameStation->iLimitNote[GetMeUserInfo()->bDeskStation];

			if (STATE_NULL != m_iUserStation[GetMeUserInfo()->bDeskStation]
				&& STATE_HAVE_DOUBLE != m_iUserStation[GetMeUserInfo()->bDeskStation]
				&& GetMeUserInfo()->bDeskStation != m_iUpGradePeople)
			{
				int iTime = 1;
				if ((m_iDoubleTime - 10) > 0)
				{
					iTime = ((rand()+GetMeUserInfo()->bDeskStation)%(10))+1;
				}
				else if (m_iDoubleTime > 0)
				{
					iTime = ((rand()+GetMeUserInfo()->bDeskStation)%(m_iDoubleTime))+1;
				} else
				{
					iTime = ((rand()+GetMeUserInfo()->bDeskStation)%(2))+1;
				}
	...
	case GS_OPEN_CARD:	//游戏进行中
		{
			...
			if (STATE_NULL != m_iUserStation[GetMeUserInfo()->bDeskStation]
				&& STATE_OPEN_CARD != m_iUserStation[GetMeUserInfo()->bDeskStation])
			{
				int iTime = 1;
...
bool CClientGameDlg::HandleGameMessage(NetMessageHead * pNetHead, void * pNetData, UINT uDataSize, CTCPClientSocket * pClientSocket)
{
	if (pNetHead->bMainID==MDM_GM_GAME_NOTIFY)
	{
		...
		case ASS_GAME_BEGIN:
			{
				m_iUserStation[GetMeUserInfo()->bDeskStation] = STATE_PLAY_GAME;
		...
		case ASS_CALL_ROBNT:
			{
				...
				RobNTStruct *pCallScore=(RobNTStruct *)pNetData;	
				if(NULL==pCallScore)
				{
					return TRUE;
				}
		...
		//		return TRUE;
		//	}
		case ASS_CALL_DOUBLE:
			{
				if (uDataSize !=sizeof(UserDoubleStruct))
				{
					return TRUE;
				}
				UserDoubleStruct * pBeginGame = (UserDoubleStruct *)pNetData;
				if (NULL == pBeginGame)
				{
					return TRUE;
				}
				//设置游戏状态
				SetStationParameter(GS_DOUBLE);

				if (STATE_NULL != m_iUserStation[GetMeUserInfo()->bDeskStation]
					&& GetMeUserInfo()->bDeskStation != pBeginGame->iNt)
				{
					int iTime = 1;
					if ((m_iDoubleTime - 10) > 0)
					{
	...
	case ID_ROB_NT_TIMER:	//抢庄定时器
		{
			...
			if(GetStationParameter() == GS_ROB_NT)
			{
				...
				RobNTStruct callScore;
				//GetMeUserInfo()->bDeskStation
				callScore.iValue=iRobValue;
				callScore.bDeskStation = GetMeUserInfo()->bDeskStation;			
				callScore.bRobNTflag = (0 == iRobValue ? false : true);
				//发送叫庄消息
				SendGameData(&callScore,sizeof(callScore),MDM_GM_GAME_NOTIFY,ASS_USER_ROBNT,0);
			}
