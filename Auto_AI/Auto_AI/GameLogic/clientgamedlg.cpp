...
// 			}
			...
			float minMultiple = max_bullet_multiple_ * 0.25f;
			float midMultiple = max_bullet_multiple_ * 0.5f;
			float bigMultiple = max_bullet_multiple_ * 0.75f;

			if (current_bullet_mulriple_ <= minMultiple)
			{
				current_bullet_kind_ = BULLET_KIND_1_ION;
			}
			else if (current_bullet_mulriple_ > minMultiple && current_bullet_mulriple_ <= midMultiple)
			{
				current_bullet_kind_ = BULLET_KIND_2_ION;
			}
			else if (current_bullet_mulriple_ > midMultiple && current_bullet_mulriple_ <= bigMultiple)
			{
				current_bullet_kind_ = BULLET_KIND_3_ION;
			}
			else
...
bool CClientGameDlg::OnSubSwitchScene(void* data, WORD data_size) 
{
	...
	OutputDebugString("XYT: OnSubSwitchScene");
...
void CClientGameDlg::Fire() 
{
	...
	CString str;
	str.Format("XYT: need_fish_score=%d, fish_score_=%d , allow_fire_=%d", need_fish_score,fish_score_, allow_fire_);
	OutputDebugString(str);
	if (fish_score_ < need_fish_score) 
	{
		OutputDebugString("XYT: ExchangeFishScore ");
		...
		{
			m_pGameRoom->SendMessage(IDM_STAND_UP,0,0);
			return ;
		}
...
float CClientGameDlg::GetAngle(WORD chair_id) 
{
	...
	else 
	{
		...
		if (chair_id == 3 || chair_id == 4 || chair_id == 5) 
		{
			angle += M_PI;
		}
	...
	case GAME_STATUS_PLAY:
		{
			...
			fish_score_ = gamestatus->fish_score[GetMeUserInfo()->bDeskStation];
			exchange_fish_score_ = 0;

			int iCount = 0;
			current_bullet_mulriple_ = CurBulletMul(min_bullet_multiple_,max_bullet_multiple_);
...
// 			}
			...
			SetGameTimer(GetMeUserInfo()->bDeskStation, TIME_EXCHANGE_FISHSCORE, IDI_EXCHANGE_FISHSCORE);		
			SetGameTimerEx(GetMeUserInfo()->bDeskStation, times*1000, IDI_FIRE);
			SetGameTimer(GetMeUserInfo()->bDeskStation, MyRand() % TIME_FIRE_INTERVAL + 20, IDI_FIRE_INTERVAL);
			return true;
		}
...
//************************************
// Method:    CurBulletMul
// FullName:  CurBulletMul
// Access:    public 
// Returns:   int
// Qualifier: 根据最小炮、最大炮设置机器人当前炮倍率
// Parameter: int min
// Parameter: int max
//************************************
int CClientGameDlg::CurBulletMul(int min,int max)
{
// 	int x =   (max <= 1000? max / 10:100);
// 	int i = rand_Mersenne(min,min + ((max > 1000)?max / 10:max));
// 	return i < x?i: (i - i % x);
