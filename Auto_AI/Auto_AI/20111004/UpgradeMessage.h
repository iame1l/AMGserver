...
/*------------------------------------------------------------------------------------------------------*/
//文件名字定义
#define GAMENAME						TEXT("红中麻将") 
#define NAME_ID						    20111004  		// 名字 ID
/*------------------------------------------------------------------------------------------------------*/
...
#define REMAIND_CARD_COUNT  59		//最多剩余59张
#define ZAMA_COUNT			9		//扎马数最多9个

#define TOTAL_CARD_COUNT	112		//总的牌数
...
enum MJTHINGS
{
	...
	S_C_UPDATE_REMAIN_JUSHU_SIG,		//已经开始的局数
    THING_MAX				            //最大事件id
};                              
...
// 	BYTE    byHuCnt;			//胡牌张数
// 	BYTE	byArHuPai[HAND_CARD_NUM];//和牌数据
	...
	void Clear()
	{
		...
		byUser	=255;				//出牌玩家位置
		byPs	=255;				//出牌的值
		//memset(byArHuPai, 255, sizeof(byArHuPai));//手牌数据
		memset(m_byArHandPai, 255, sizeof(m_byArHandPai));//手牌数据	
		memset(m_byArHandPaiCount, 0 ,sizeof(m_byArHandPaiCount));	//手牌张数
...
/*----------------------------------------------------------------*/
// /出牌事件
// struct	tagTingHuPaiMsg
// {
//  	BYTE	byUser;				//出牌玩家位置
// 	BYTE	byHuPaiCard[HAND_CARD_NUM][HAND_CARD_NUM];	//出牌的值和胡牌数据(byHuPaiCard[HAND_CARD_NUM][0]听牌后能出的牌，[1]-[HAND_CARD_NUM]对应的能胡牌数据)
// 	void Clear()
// 	{
// 		byUser	=255;//听牌玩家位置
// 		memset(byHuPaiCard,255,sizeof(byHuPaiCard));//玩家点击了听牌
// 	}
// };
/*----------------------------------------------------------------*/
///胡牌信息
...
#define ASS_CHANGE_PAI_RESULT           163             //用户换牌结果
#define ASS_GET_RAMAINNING_PAI			164				//超端读取牌墙
#define ASS_IM_SUPERMAN					165				//通知客户端自己是超端
/********************************************************************************/
