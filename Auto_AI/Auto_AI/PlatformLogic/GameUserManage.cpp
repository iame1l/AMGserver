...
//	INT_PTR uActiveCount=m_ActiveUserPtrArray.GetCount();
	//for (int i=0;i<uActiveCount;i++)
	//{
		UserItemStruct * pFindUserItem=(UserItemStruct *)m_ActiveUserPtrArray.GetAt(index);
...
//		if ((pFindUserItem!=NULL)&&(pFindUserItem->GameUserInfo.dwUserID==dwAccID)) return pFindUserItem;
//	}
//	return NULL;
//}
//查找用户
UserItemStruct * CGameUserManage::FindOnLineUser(long int dwUserID)
{
	...
	{
		UserItemStruct * pFindUserItem=(UserItemStruct *)m_ActiveUserPtrArray.GetAt(i);
		if ((pFindUserItem!=NULL)&&(pFindUserItem->GameUserInfo.dwUserID==dwUserID)) return pFindUserItem;
	}
	return NULL;
}
