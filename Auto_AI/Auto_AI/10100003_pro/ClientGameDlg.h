...
private:
	//可出手牌集合
	vector<Ecardgroup*> analyse;//分析后拆分的牌型集合

	Ecardgroup canoutCard;

	//转换
	int bytetoint(BYTE cardValue);

	int ValueToNum(set<int> &cardscopy, int value);
	//手牌中为0的卡牌
	void freezoro(map<int, int> &Handcard);
	//算手牌
	bool HandcardtoList(const set<int> Handcard);

	void DeleteUnkown(void);
	bool MyCompare(Ecardgroup *c1, Ecardgroup *c2);

	void sandaiXorfeijidaiX();

	void BankeroutCard(bool isnormal);

	void frameroutCard(int nexthandcard,bool isfriend);
