...
#define UG_TWO_DOUBLE							3				//两对
#define UG_VARIATION_STRAIGHT					4				//变种顺子(A2345)顺子中最小
#define UG_STRAIGHT								5               //顺子,5+张连续牌
#define UG_FLUSH								6				//同花(非连)
#define UG_STRAIGHT_FLUSH						7               //同花顺,花色相同的顺子
...
#define UG_THREE_ONE_SEQUENCE					17              //2+个连续的三带一(三顺带两个单牌)
...
#define UG_VARIATION_FOUR_SEQUENCE				28				//四顺
...
#define UG_BOMB									40              //炸弹>=4張
#define UG_BOMB_SAME_HUA						41				//同花炸弹(在四副或以上的牌中出现)
#define UG_KING_BOMB							42				//王炸(最大炸弹)


#define KING_COUNT								4				//所有王的个数
//510k牌型信息
//const int i510kCardStyle[19]={UG_ONLY_ONE,UG_STRAIGHT,UG_DOUBLE,UG_TWO_DOUBLE,UG_DOUBLE_SEQUENCE,UG_THREE,UG_THREE_SEQUENCE,
//UG_THREE_ONE_SEQUENCE,UG_THREE_SEQUENCE_DOUBLE_SEQUENCE,UG_BOMB,UG_THREE_ONE,UG_THREE_DOUBLE,UG_SLAVE_510K,UG_MASTER_510K,UG_STRAIGHT_FLUSH
//,UG_FOUR_TWO,UG_FOUR_TWO_DOUBLE,UG_THREE_DOUBLE_SEQUENCE,UG_KING_BOMB};

const int i510kCardStyle[] = {UG_ONLY_ONE,UG_DOUBLE,UG_TWO_DOUBLE,UG_DOUBLE_SEQUENCE,UG_STRAIGHT,
	/*UG_THREE,UG_THREE_SEQUENCE,UG_THREE_ONE,UG_THREE_ONE_SEQUENCE,*/UG_THREE_TWO,UG_THREE,UG_THREE_SEQUENCE,UG_THREE_TWO_SEQUENCE,
	UG_SLAVE_510K,UG_MASTER_510K,UG_BOMB,UG_KING_BOMB};
//扑克分析结构	（只用于服务器对比扑克）
...
public:
	...
	//切换排序(大小排序和牌型排序之间切换)
	void ChangeSort(BYTE iCardList[],BYTE iCardCount);
public:
	//混乱扑克
	BYTE RandCard(BYTE iCard[], int iCardCount,int iRoomId,bool bHaveKing=false);		//bHaveKing表示是否有大小猫,false无,ture有
	//删除扑克
	BOOL RemoveCard(BYTE iRemoveCard[], int iRemoveCount, BYTE iCardList[], int iCardCount);
private:
	//清除 0 位扑克
...
public://[辅助函数]
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////
	bool IS2Bomb(BYTE iCardList[],int iCardCout);
	//[牌型相关]
public:
    //获取牌型
	BYTE GetCardShape(BYTE iCardList[], int iCardCount,int iHandCardCount = 0,bool bExlVal=false);
	//是否单牌
	...
	//是否两对
	BOOL IsTwoDouble(BYTE iCardList[], int iCardCount);
...
public:	
	//提取连续的两对
	BOOL TackOutSecondDouble(BYTE iCardList[], int iCardCount, BYTE iBaseCard[], int iBaseCount, BYTE iResultCard[], int &iResultCardCount);
...
public:
	...
	//设置有分牌(510k)
	void Set510KPoint(BYTE bBaseCardList[],int iBaseCount,BYTE b510kList[],int i510KCount);
	//计算得分(510k)
	__int64 Get510kPoint(BYTE bCardList[],int iCount);
	//是否是有分牌
	BOOL Is510K(BYTE bCardData);

	//是否黑桃5
	BOOL IsSpade_5(BYTE bCardData);
	//是否红桃5
	BOOL IsRed_5(BYTE bCardData);
	//是否梅花5
	BOOL IsFlower_5(BYTE bCardData);
	//是否方块5
	BOOL IsBox_5(BYTE bCardData);

	//是否黑桃10
	BOOL IsSpade_10(BYTE bCardData);
	//是否红桃10
	BOOL IsRed_10(BYTE bCardData);
	//是否梅花10
	BOOL IsFlower_10(BYTE bCardData);
	//是否方块10
	BOOL IsBox_10(BYTE bCardData);

	//是否黑桃k
	BOOL IsSpade_k(BYTE bCardData);
	//是否红桃k
	BOOL IsRed_k(BYTE bCardData);
	//是否梅花k
	BOOL IsFlower_k(BYTE bCardData);
	//是否方块k
	BOOL IsBox_k(BYTE bCardData);

	//判断是否五十k牌型
	BOOL Is510kCardStyle(int iCardStyle);

	BOOL IsThree(BYTE iCardList[], int iCardCount,bool bExtVal=false);
	BOOL IsThreeSequence(BYTE iCardList[],int iCardCount,int iHandCardCout);
	//验证游戏中数据是否有效
