...
void PlayCardListener::OnAdd()
{
	...
	extTimer->Add((float)Config::iXiaZhuTime+1.f,TimerData(pointers,ARRAY_LEN(pointers),DelayPlayCardOver));
...
void PlayCardListener::notiBeginPlay()
{
	...
	for (int i=0;i<PLAY_COUNT; ++i)
	{
		if(exDataMgr->isNomalUser(i))
		{
			...
			m_Context->GetGameDesk()->send2(i,(char*)&Noti,sizeof(Noti),S_C_NOTICE_XIA_ZHU);
		}	
	}
	m_startTime = 0;
}
