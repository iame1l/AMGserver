...
#define UG_THREE_SEQUENCE_EX					23				//333444555666A 或 333444555777 或 333444555776 型

//#define UG_FOUR_ONE								23				//四带一
//#define UG_FOUR_TWO								24				//四带二张
//#define UG_FOUR_ONE_DOUBLE						25				//四带一对
//#define UG_FOUR_THREE							26				//四带二对
//
...
//#define UG_FOUR_THREE_SEQUENCE					36				//四带三张
...
//	BYTE	iLesssingleCard;  //最小单牌
	...
	BYTE	iSingleArray[CARD_COUNT]; //单牌
	BYTE	iDoubleArray[CARD_COUNT]; //对牌数组
	BYTE	iThreeArray[CARD_COUNT];  //三条数组
	BYTE	iBombArray[CARD_COUNT];	  //两对拖拉机数组 （0 分割，长度，拖拉机数据）
	BYTE	i510KArray[CARD_COUNT];	  //三对拖拉机数组 （0 分割，长度，拖拉机数据）
};
...
public:
	...
	//混乱扑克(15张打法)
	BYTE RandCard15(BYTE iCard[], int iCardCount,int iRoomId,bool bHaveKing=false);		//bHaveKing表示是否有大小猫,false无,ture有
...
public://[辅助函数]
	...
	//是否为某指定的顺子特殊的
	bool IsSequenceEx(BYTE iCardList[], int iCardCount, int iCount,int iDelete = 0);
	//提取指定的牌
	...
	//获取最大的连队
	bool GetMaxThreeSequence(BYTE iCardList[], int iCardCount,int iCount,BYTE byBuff[],int &iBuffCount);
	////////////////////////////////////////////////////////////////////////////////////////////////////
	//[牌型相关]
public:
    //获取牌型
...
public:	
   ...
   ///提取指定的牌
	...
	//提取指定长度的顺子
	bool TackSequenceLen(BYTE byCardList[],int iCardCount,int iCount,BYTE byBuff[],int &iOut,int DeleteCount);
...
public:
	//检测当前出的牌是否与手牌组成炸弹
	bool  CheckOutCardAndHandCardComBo(BYTE byOutCard[],int iOutCardCount,BYTE byHandCard[],int iHandCardCount);

	//检测开局第一个要出黑桃3
	bool  CheckHaveBlackThree(BYTE byOutCard[],int iOutCardCount);
