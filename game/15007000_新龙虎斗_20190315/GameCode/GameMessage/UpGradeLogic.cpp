...
#define GetLengh(a) (sizeof(a)/sizeof(a[0]))
//构造函数
...
//切换排序(大小排序和牌型排序之间切换)
void CUpGradeGameLogic::ChangeSort(BYTE iCardList[],BYTE iCardCount)
{
	m_bSortCardStyle = !m_bSortCardStyle;
...
BYTE CUpGradeGameLogic::RandCard(BYTE iCard[], int iCardCount,int iRoomId,bool bHaveKing)
{
	...
	{
		0x01, 0x02 ,0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, //黑桃 2 - A
		...
		0x31, 0x32 ,0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, //方块 2 - A
		0x4E, 0x4F //小鬼，大鬼
	};
...
//删除扑克
BOOL CUpGradeGameLogic::RemoveCard(BYTE iRemoveCard[],   //要删除的牌面
								  int iRemoveCount,		//要删除的牌总数
								  BYTE iCardList[],		//要处理的数组
								  int iCardCount)		//处理数组的上限
{
	//检验数据
	if(iRemoveCount > iCardCount) return false;
	...
	RemoveNummCard(iCardList, iCardCount); //删除做了标记的牌
	return true;
}
...
int CUpGradeGameLogic::RemoveNummCard(BYTE iCardList[], int iCardCount)
{
	...
	for (int i=0;i<iCardCount;i++)
	{
		if (TestData(iCardList[i],1))
		{
			bCopyList[iCopyCount]=iCardList[i];
			iCopyCount++;
		}
	}
	memset(iCardList,0,sizeof(BYTE)*iCardCount);
	CopyMemory(iCardList,bCopyList,sizeof(BYTE)*iCopyCount);
	return iCopyCount;
}
...
BOOL CUpGradeGameLogic::IsSameNumCard(BYTE iCardList[],int iCardCount,bool bExtVal)
{
	...
	if(m_bKingCanReplace)
	{
		...
		return (temp[i]==iCardCount);//修改有炸弹时王可以百搭
	return 0;
}
...
BYTE CUpGradeGameLogic::GetBombCount(BYTE iCardList[],int iCardCount,int iNumCount,bool bExtVal)
{
	...
	for(int i=0;i<16;i++)
	{
		if(temp[i] >= iNumCount || (temp[i]>=4 && temp[i]+temp[16]+temp[17] > iNumCount))
			iCount++;
	}
	return iCount;	
}
													...
													bool bExtVal)
{
	...
	int iExtVal = bExtVal?16:18;
	for(int i=0; i<iExtVal; i++)
	{
...
BOOL CUpGradeGameLogic::TackOutCardBySpecifyCardNum(BYTE iCardList[],int iCardCount,BYTE iBuffer[],int &iBufferCardCount,BYTE iCard,BOOL bExtVal)
{
	...
	for(int i = 0; i < iCardCount; i++)
		byTemp[GetCardBulk(iCardList[i])]++;
	...
	if(iCardCount >= 5 && iBufferCardCount == 3)
	{
		iBuffer[iBufferCardCount++] = iCardList[iCardCount - iBufferCardCount - 1];
		iBuffer[iBufferCardCount++] = iCardList[iCardCount - iBufferCardCount - 1];
	}
	else if(iCardCount >= 4 && iBufferCardCount == 3)
	{
		iBuffer[iBufferCardCount++] = iCardList[iCardCount - iBufferCardCount - 1];
	}
	else if(GetCardNum(iCardList[2]) == 5 && GetCardNum(iCardList[1]) == 10 && GetCardNum(iCardList[0]) == 13 && iCardCount == 3)
	{
		iBuffer[iBufferCardCount++] = iCardList[0];
		iBuffer[iBufferCardCount++] = iCardList[1];
		iBuffer[iBufferCardCount++] = iCardList[2];
	}
...
* @param bExlVol ?
*/
BYTE CUpGradeGameLogic::GetCardShape(BYTE iCardList[], int iCardCount,int iHandCardCount,bool bExlVol)
{
	...
	if (IsTwoDouble(iCardList,iCardCount)) return UG_TWO_DOUBLE;//连续的两对
	if (IsDoubleSequence(iCardList, iCardCount)) return UG_DOUBLE_SEQUENCE;  //连对

	if (IsThree(iCardList,iCardCount)) return UG_THREE;	//三张
	...
	//if (IsThreeXSequence(iCardList, iCardCount, 1)) return UG_THREE_ONE_SEQUENCE; //连的三带一
 	...
 	if (IsThreeSequence(iCardList, iCardCount, iHandCardCount)) return UG_THREE_TWO_SEQUENCE; //连的三带二
...
//是否两对
BOOL CUpGradeGameLogic::IsTwoDouble(BYTE iCardList[], int iCardCount)
{
	if(4!=iCardCount)
	{
		return false;
...
BOOL CUpGradeGameLogic::IsBomb(BYTE iCardList[], int iCardCount, bool bExtVal)
{
	...
	for(int i = 0; i<iCardCount; i++)
	{
		if(GetCardBulk(iCardList[i]) > 15)
			kindCount++;
	}
	if(iCardCount - kindCount < 4)
		return FALSE;
	...
	return ((Temp[iStation]+Temp[16]+Temp[17]) == iCardCount);
	//return IsSameNumCard(iCardList, iCardCount - kindCount, bExtVal); //是否是相同数字
}
...
BOOL CUpGradeGameLogic::IsSlave510K(BYTE iCardList[], int iCardCount, bool bExtVal)
{
	...
	if(1==(Test[5] && 1==Test[10] && 1==Test[13]))
	{
		return true;
	}
	else
												...
												bool bExtVal)
{
	BYTE iTempCard[54] = {0};
	iResultCardCount = 0;
	int iBaseShape = GetCardShape(iBaseCard, iBaseCardCount,iBaseCardCount); //桌面上牌的牌型
	//	TCHAR sz[200];
	...
	//case UG_BOMB:	//四张 炸弹
		{
			...
			if(iCount > 0)
			{
				BYTE Step = GetSerialByMoreThanSpecifyCard(iTempCard, iCount, iBaseCard[iBaseCardCount -1], iBaseCardCount, false);
				CopyMemory(iResultCard, &iTempCard[Step], sizeof(BYTE)*iBaseCardCount);	

				if(CompareOnlyOne(iBaseCard[iBaseCardCount -1], iResultCard[iBaseCardCount -1]))
				{
	...
	case UG_BOMB:	//四张 炸弹
		{
			...
			int BombNum = (iBaseCardCount-iKingCount < 4)?4:(iBaseCardCount-iKingCount);
			BYTE iCount = TackOutBySepcifyCardNumCount(iHandCard, iHandCardCount, iTempCard, BombNum,true);

			if(iCount > 0)
			{
				iKingCount = iBaseCardCount - BombNum;
				int iKingNum(0);
				for(int i = 0; i < iHandCardCount; i++)
				{
					if(GetCardNum(iHandCard[i]) > 14)
					{
						iResultCard[iKingNum++] = iHandCard[i];
						if(iKingNum == iKingCount)
							break;
					}
				}

				BYTE Step = GetSerialByMoreThanSpecifyCard(iTempCard, iCount, iBaseCard[iBaseCardCount -1], BombNum, false);
				CopyMemory(iResultCard + iKingCount, &iTempCard[Step], sizeof(BYTE)*BombNum);
	...
	case UG_TWO_DOUBLE://两对
		{
			//提取连续的两对
			if(TackOutSecondDouble(iHandCard, iHandCardCount,iBaseCard,iBaseCardCount,iResultCard, iResultCardCount))
			{
				return true;
			}
			break;
	...
	case UG_THREE_ONE: //三带一
		{
			TackOutThreeX(iHandCard, iHandCardCount, iBaseCard, iBaseCardCount, iResultCard, iResultCardCount,1);
			if(iResultCardCount>0)
			{
				return true;
			}
	...
	case UG_THREE_TWO: //三带二张
		{
			TackOutThreeX(iHandCard, iHandCardCount, iBaseCard, iBaseCardCount, iResultCard, iResultCardCount,2);
			if(iResultCardCount>0)
			{
				return true;
			}
	...
	case UG_THREE_DOUBLE:	//三带一对
		{
			TackOutThreeX(iHandCard, iHandCardCount, iBaseCard, iBaseCardCount, iResultCard, iResultCardCount,3);
			if(iResultCardCount>0)
			{
				return true;
			}
	...
	case UG_STRAIGHT: //顺子
		{
			TackOutStraightFlush(iHandCard, iHandCardCount, iBaseCard, iBaseCardCount, iResultCard, iResultCardCount);
			if(iResultCardCount>0)
			{
				return true;
			}
			TackOutSequence(iHandCard, iHandCardCount, iBaseCard, iBaseCardCount, iResultCard, iResultCardCount,1);
			if(iResultCardCount>0)
			{
				return true;
			}
	...
	case UG_DOUBLE_SEQUENCE: //连对
		{
			TackOutSequence(iHandCard, iHandCardCount, iBaseCard, iBaseCardCount, iResultCard, iResultCardCount,2);
			if(iResultCardCount>0)
			{
				return true;
			}
	...
	case UG_THREE_SEQUENCE: //连三
		{
			TackOutSequence(iHandCard, iHandCardCount, iBaseCard, iBaseCardCount, iResultCard, iResultCardCount,3);
			if(iResultCardCount>0)
			{
				return true;
			}
	...
	case UG_THREE_ONE_SEQUENCE: //三带一的连牌
		{
			TrackOut3XSequence(iHandCard, iHandCardCount, iBaseCard, iBaseCardCount, iResultCard, iResultCardCount,1);
			if(iResultCardCount>0)
			{
				return true;
			}
	...
	case UG_THREE_TWO_SEQUENCE: //三带二的连牌
		{
			TrackOut3XSequence(iHandCard, iHandCardCount, iBaseCard, iBaseCardCount, iResultCard, iResultCardCount,2);
			if(iResultCardCount>0)
			{
				return true;
			}
	...
	case UG_THREE_DOUBLE_SEQUENCE://三带对连牌
		{
			TrackOut3XSequence(iHandCard, iHandCardCount, iBaseCard, iBaseCardCount, iResultCard, iResultCardCount,3);
			if(iResultCardCount>0)
			{
				return true;
			}
	...
	case UG_THREE_SEQUENCE_DOUBLE_SEQUENCE:
		{
			TrackOut3Sequence2Sequence(iHandCard, iHandCardCount, iBaseCard, iBaseCardCount, iResultCard, iResultCardCount);
			if(iResultCardCount>0)
			{
				return true;
			}
	...
	if(iResultCardCount == 0) //没找到同牌型的大牌,就找大一点的牌型
	{
		...
		case UG_DOUBLE:
			{
				TackOutCardByNoSameShape(iHandCard, iHandCardCount, iResultCard, iResultCardCount, iBaseCard, iBaseCardCount);
				if(iResultCardCount>0)
				{
					return true;
				}
		...
		case UG_BOMB:
			{
				...
				TackOutBomb(iHandCard, iHandCardCount, iResultCard, iResultCardCount, iBaseCardCount+1);
				if(iResultCardCount>0)
				{
					return true;
				}
	...
	//提取510k牌
	iResultCardCount=0;
	if(0==iResultCardCount)
	{
		if(iBaseShape<UG_SLAVE_510K)
		{
			TrackOut510K(iHandCard,iHandCardCount,iResultCard,iResultCardCount);
			if(iResultCardCount>0)
			{
				return true;
			}
...
{
	iResultCardCount = 0;
	int ishape = GetCardShape(iBaseCard, iBaseCardCount,iBaseCardCount);
	switch(ishape)
	{
...
//提取连续的两对
BOOL CUpGradeGameLogic::TackOutSecondDouble(BYTE iCardList[], int iCardCount, BYTE iBaseCard[], int iBaseCount, BYTE iResultCard[], int &iResultCardCount)
{
	if(4!=iBaseCount)
	{
		iResultCardCount=0;
	...
	int iBaseNums=0,iTackOutNums=0, temp[18] = {0};
	memset(temp,0,sizeof(temp));
	for(int i = 0; i < iCardCount; i++)
	{
	...
	{
		iBaseNums+=GetCardNum(iBaseCard[i]);
	}
	for(int i=0; i<18; i++)
	{
		iTackOutNums=0;
		if(temp[i]>=2)//找到一对
		{
			if((i+1)<18)
			{
				if(temp[i+1]>=2)//找到连续的两对
				{
					//比较相对牌大小
					iTackOutNums=(i*2)+((i+1)*2);
					if(iTackOutNums>iBaseNums)
					{
						int iTemp_1=0;
						for(int j=0;j<18;j++)
						{
							if(i==GetCardNum(iCardList[j]))
							{
								iResultCard[iTemp_1]=iCardList[j];
								iTemp_1++;
							}
							if(2==iTemp_1)
							{
...
BOOL CUpGradeGameLogic::CanOutCard(BYTE iOutCard[], int iOutCount,  //要出的牌
								   ...
								   bool bFirstOut)
{
	BYTE iOutCardShape = GetCardShape(iOutCard, iOutCount,iHandCount);
	...
	BYTE iBaseCardShape = GetCardShape(iBaseCard, iBaseCount,iBaseCount); //桌面上的牌型

	if(iBaseCardShape > iOutCardShape)						//牌形<
	{
		return FALSE;
	}
	if(iOutCardShape == UG_KING_BOMB)
		return true;
	if( iBaseCardShape < iOutCardShape)						//牌形>
	{
		if(iBaseCardShape <UG_SLAVE_510K && iOutCardShape>=UG_SLAVE_510K)
		{
			return true;
		}
		if(iOutCardShape<UG_SLAVE_510K &&  iBaseCardShape>=UG_SLAVE_510K)
		{
			return false;
		}
		switch(iBaseCardShape)
		{
		case UG_SLAVE_510K:												//510K炸弹,花色不同
		case UG_MASTER_510K:											//510K同花炸弹
		case UG_BOMB:									             //炸弹>=4張
		case UG_BOMB_SAME_HUA:									     //同花炸弹(在四副或以上的牌中出现)
		//case UG_KING_BOMB:											//王炸(最大炸弹)
			{
	...
	case UG_TWO_DOUBLE://两对
		{
			...
			{
				temp_1+=GetCardNum(iOutCard[i]);
			}
			for(int i=0;i<iBaseCount;i++)
			{
				temp_2+=GetCardNum(iBaseCard[i]);
			}

			return (temp_1>temp_2);
		}
	
	case UG_BOMB_SAME_HUA:									     //同花炸弹(黑桃>红桃>草花>方片)
		{
	...
	case UG_MASTER_510K: //同花510K都一样大
		{
			return FALSE;//(GetCardHuaKind(iBaseCard[0],true) < GetCardHuaKind(iOutCard[0],true)); //比花色
		}
									...
									BYTE iResultCard[], int &iResultCardCount,int iNumCount)
{
	...
	for(int i = 0; i < iCardCount;i++)
	{
		bTemp[GetCardBulk(iCardList[i])]++;
	}
	int iKingCount = bTemp[16] + bTemp[17];

	int realNum = iNumCount - iKingCount < 4 ? 4:(iNumCount - iKingCount);

	for(int i=realNum;i<9;i++)
	{
		int count=TackOutBySepcifyCardNumCount(iCardList,iCardCount,bCardBuffer,i);
		if(count > 0)
		{
			memcpy(iResultCard,iCardList,sizeof(BYTE)*iKingCount);
			::CopyMemory(iResultCard+sizeof(BYTE)*iKingCount,bCardBuffer,sizeof(BYTE)*i);
			
			iResultCardCount=i + iKingCount;
			break;
		}
	}
...
//设置有分牌
void CUpGradeGameLogic::Set510KPoint(BYTE bBaseCardList[],int iBaseCount,BYTE b510kList[],int i510KCount)
{
	for(int i=0;i<iBaseCount;i++)
	{
		switch(GetCardNum(bBaseCardList[i]))
		{
		case 5:
			{
				for(int j=0;j<i510KCount;j++)
				{
					if(!TestData(b510kList[j],1))
					{
						b510kList[j]=bBaseCardList[i];
						break;
					}
				}
				break;
...
//计算得分(510k)
__int64 CUpGradeGameLogic::Get510kPoint(BYTE bCardList[],int iCount)
{
	__int64 i64Point=0;
...
//判断是否五十k牌型
BOOL CUpGradeGameLogic::Is510kCardStyle(int iCardStyle)
{
	for(int i=0;i<GetLengh(i510kCardStyle);i++)
	{
		if(iCardStyle==i510kCardStyle[i])
		{
			return true;
		}
	}
...
//是否是有分牌
BOOL CUpGradeGameLogic::Is510K(BYTE bCardData)
{
	
	//是否黑桃5
	if(IsSpade_5(bCardData)) return true;
	//是否红桃5
	if(IsRed_5(bCardData)) return true;
	//是否梅花5
	if(IsFlower_5(bCardData)) return true;
	//是否方块5
	if(IsBox_5(bCardData)) return true;

	//是否黑桃10
	if(IsSpade_10(bCardData)) return true;
	//是否红桃10
	if(IsRed_10(bCardData)) return true;
	//是否梅花10
	if(IsFlower_10(bCardData)) return true;
	//是否方块10
	if(IsBox_10(bCardData)) return true;

	//是否黑桃k
	if(IsSpade_k(bCardData)) return true;
	//是否红桃k
	if(IsRed_k(bCardData)) return true;
	//是否梅花k
	if(IsFlower_k(bCardData)) return true;
	//是否方块k
	if(IsBox_k(bCardData)) return true;
	return false;
}

//是否黑桃5
BOOL CUpGradeGameLogic::IsSpade_5(BYTE bCardData)
{
	if(0x04==bCardData)
	{
		return true;
...
//是否红桃5
BOOL CUpGradeGameLogic::IsRed_5(BYTE bCardData)
{
	if(0x14==bCardData)
	{
		return true;
...
//是否梅花5
BOOL CUpGradeGameLogic::IsFlower_5(BYTE bCardData)
{
	if(0x24==bCardData)
	{
		return true;
...
//是否方块5
BOOL CUpGradeGameLogic::IsBox_5(BYTE bCardData)
{
	if(0x34==bCardData)
	{
		return true;
...
//是否黑桃10
BOOL CUpGradeGameLogic::IsSpade_10(BYTE bCardData)
{
	if(0x09==bCardData)
	{
		return true;
...
//是否红桃10
BOOL CUpGradeGameLogic::IsRed_10(BYTE bCardData)
{
	if(0x19==bCardData)
	{
		return true;
...
//是否梅花10
BOOL CUpGradeGameLogic::IsFlower_10(BYTE bCardData)
{
	if(0x29==bCardData)
	{
		return true;
...
//是否方块10
BOOL CUpGradeGameLogic::IsBox_10(BYTE bCardData)
{
	if(0x39==bCardData)
	{
		return true;
...
//是否黑桃k
BOOL CUpGradeGameLogic::IsSpade_k(BYTE bCardData)
{
	if(0x0C==bCardData)
	{
		return true;
...
//是否红桃k
BOOL CUpGradeGameLogic::IsRed_k(BYTE bCardData)
{
	if(0x1C==bCardData)
	{
		return true;
...
//是否梅花k
BOOL CUpGradeGameLogic::IsFlower_k(BYTE bCardData)
{
	if(0x2C==bCardData)
	{
		return true;
...
//是否方块k
BOOL CUpGradeGameLogic::IsBox_k(BYTE bCardData)
{
	if(0x3C==bCardData)
	{
		return true;
...
bool CUpGradeGameLogic::IS2Bomb(BYTE iCardList[],int iCount)
{
	BYTE Temp[18] = {0};
	for(int i = 0; i < iCount; i++)
	{
...
BOOL CUpGradeGameLogic::IsThree(BYTE iCardList[], int iCardCount,bool bExtVal)
{
	if (iCardCount!=3)
		return FALSE;
	return IsSameNumCard(iCardList,iCardCount,bExtVal);
...
BOOL CUpGradeGameLogic::IsThreeSequence(BYTE iCardList[],int iCardCount,int iHandCardCout)
{
	if(iCardCount < 6)		//三顺至少2
		return false;
	...
	int TackOutCount(0);
	TackOutCount=TackOutBySepcifyCardNumCount(iCardList,iCardCount,iBuffer,3);
	if(TackOutCount>0 && (TackOutCount/3*5 == iCardCount || iCardCount == iHandCardCout))
		return IsSequence(iBuffer,TackOutCount,3);
