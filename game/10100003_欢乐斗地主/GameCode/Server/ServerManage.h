...
#define TIME_LEAVE                  42              //超时不准备踢人时间

#define TIME_OUT_CARD_DELAY			50				//延迟出牌

//游戏结束标志定义
#define GF_NORMAL					10				//游戏正常结束
#define GF_SAFE						11				//游戏安全结束
#define GF_NO_CALL_SCORE			16				//无人叫分
...
protected:
    ...
    BYTE                    m_iOutTimeCount[PLAY_COUNT];
	...
	CurCalcBoardInf			m_CulCalcInf[PLAY_COUNT];

protected:
	static int				m_iTimeOutNoCardOut;						//到达时间是否可以不出牌（1为可以不出牌）
	static int				m_iModel;								//抢地主还是叫分模式(1为开启抢分模式)
...
public:
	...
	/******************************创建房间相关****************************/
	//大结算消息
	void	UpdateCalculateBoard(BYTE bCloseFlag);
	//读文件
	bool readFromFile(const string& filename,string & strbuf);

	//当前局信息累积到大结算
	bool cul2TotalInf();
#ifdef _CREATEROOM_PARA_JSON
	//解析创建房间的配置项json格式
	bool parse(const string& strSrc);
#endif // _CREATEROOM_PARA_JSON
...
public:
	...
	//int						m_CachedUserID[PLAY_COUNT];
	//CString					m_RecordFilePath;		//用户目录
	//加载配牌
	void LoadPeiPai();
	//记录出牌
	void SetRecorder(BYTE bDeskStation,  BYTE iOutCard[], int iCardCount);
	//获取配置值
	unsigned char GetCardValue(const CString &cardListStr, int cnt) ;
private:
	TOutCardRecorder		m_tOutCardRecorder;
