...
#include "writelog.h"
#include "TickCount/MyTickCount.h"
#include <math.h>
//游戏配置
...
BYTE CServerGameDesk::m_iOutTimes;
int CServerGameDesk::m_iModel;
int CServerGameDesk::m_iTimeOutNoCardOut;
...
CServerGameDesk::CServerGameDesk(void):CGameDesk(FULL_BEGIN)
{
    ...
    ::memset(m_iOutTimeCount, 0, sizeof(m_iOutTimeCount));
	::memset(m_RecordTime, 0, sizeof(m_RecordTime));
...
BOOL CServerGameDesk::LoadIni()
{
	...
	m_iModel = f.GetKeyVal(key, "Model", 0);
...
BOOL CServerGameDesk::LoadExtIni()
{
    ...
    m_iOutTimes = f.GetKeyVal(key, "OutTimes", 1);
	if(m_iBaseMult <=0)
	{
		m_iBaseMult = 1 ; 
	}
	...
	m_bRecorderSwitch = f.GetKeyVal(key,"RecorderSwitch", 0);
...
bool CServerGameDesk::HandleNotifyMessage(BYTE bDeskStation, NetMessageHead * pNetHead, void * pData, UINT uSize, UINT uSocketID, bool bWatchUser)
{
	...
	case ASS_BEGIN_TIME:
		...
		fclose(fp);
			...
			SendGameDataEx(bDeskStation, &time, sizeof(time), MDM_GM_GAME_NOTIFY, ASS_BEGIN_TIME, 0);		
			return true;
		}
	case C_S_RECORDER_REQ:
		{
			if(m_bGameStation <= GS_WAIT_ARGEE || m_bGameStation >= GS_WAIT_NEXT)
			{
				return true;
			}
			if(!m_bRecorderSwitch)
			{
				return true;
			}
			T_S_C_RECORDER_RSP tRecorderRsp;
			memcpy(&tRecorderRsp, &m_tOutCardRecorder, sizeof(m_tOutCardRecorder));
			SendGameDataEx(bDeskStation, &tRecorderRsp, sizeof(tRecorderRsp), MDM_GM_GAME_NOTIFY, S_C_RECORDER_RSP, 0);		
			break;
		}
	case ASS_GM_AGREE_GAME:		//用户同意游戏
		{
			...
			KillTimer(TIME_LEAVE + bDeskStation);
			if(uSize == sizeof(UserMingStruct))
			{
				...
				CString str;
				str.Format("chenlog----用户同意游戏-[%s]  %d",m_pUserInfo[bDeskStation]->m_UserData.nickName,m_GameMutiple.sMingPaiMutiple[bDeskStation] );
	...
	case TIME_OUT_CARD:
		{
			...
			if(m_bGameStation == GS_PLAY_GAME) 
			{
				if (1 == m_iTimeOutNoCardOut)
				{
					break;
				} 
				else
				{
                    int i = m_iOutCardPeople;
                    m_iOutTimeCount[i]++;
					UserAutoOutCard(m_iOutCardPeople);
                    if (m_iOutTimeCount[i] >= m_iOutTimes)
                        UserSetAuto(i, true);
				}
			}
			break;
		}
	case TIME_OUT_CARD_DELAY:
		{
			KillTimer(TIME_OUT_CARD_DELAY);
			UserOutCardNotify();
		} break;
    ...
    case  TIME_LEAVE+2:
       {
        KillTimer(uTimerID);
        if (!m_bIsBuy)
            UserLeftDesk(uTimerID - TIME_LEAVE, m_pUserInfo[uTimerID - TIME_LEAVE]);
		break;
       }
...
bool CServerGameDesk::OnGetGameStation(BYTE bDeskStation, UINT uSocketID, bool bWatchUser)
{
	...
	if (!bWatchUser)
	{
		m_iOutTimeCount[bDeskStation] = 0;
	...
	if(m_iDeskID == 255)
	{
		if(m_pUserInfo[bDeskStation] != NULL && m_pDataManage != NULL) 
		{
			m_iDeskID = m_pUserInfo[bDeskStation]->m_UserData.bDeskNO;
			m_iRoomID = m_pDataManage->m_InitData.uRoomID;
		}
	}
	...
	for (int i = 0; i < PLAY_COUNT; i++)
	{
		...
		SendGameDataEx(i,&iLeaveJuShu,sizeof(int),MDM_GM_GAME_NOTIFY,S_C_UPDATE_REMAIN_JUSHU_SIG,0);
        SendGameDataEx(i, &bDeskStation, sizeof(BYTE),MDM_GM_GAME_NOTIFY, S_C_USER_SIT, 0 );
	}
    if (!m_bIsBuy&& !IsPlayGame(bDeskStation))
	{
        SetTimer(TIME_LEAVE + bDeskStation, m_iBeginTime * 1000);
		m_RecordTime[bDeskStation] = CMyGetTickCount::GetTime();
	}
			...
			case  GS_FLAG_ROB_NT:
				{
					for(int  i = 0 ; i<PLAY_COUNT ; i++)
					{
						if(m_iFirstRobNt!= i) 
						{
							...
							{
								GameStation.iRobNT[i] = 2 ; ///不抢
							}
							else if(m_iRobStation[i] >0 &&m_GameMutiple.sRobNtMutiple[i] >0)
							{
...
bool CServerGameDesk::ReSetGameState(BYTE bLastStation)
{
	...
	{
		m_iAwardPoint[i]=0;
		SetTimer(TIME_LEAVE + i,  m_iBeginTime * 1000);
		m_RecordTime[i] = CMyGetTickCount::GetTime();
	}
	m_iGameFlag = GS_FLAG_NORMAL;
	...
	memset(m_CulCalcInf,0,sizeof(m_CulCalcInf));
	m_tOutCardRecorder.InitObject();
	return TRUE;
}
...
//************************************
	...
	// FullName:  CServerGameDesk::CreateNewRecord
	// Access:    public 
	// Returns:   bool
	// Qualifier:
	//************************************
bool CServerGameDesk::CreateNewRecord()
{
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
	{
		...
		//文件名 时间-用户ID.dat
		//sprintf(fileName,"%s%d-%d.dat",m_RecordFilePath,GetTickCount(),m_pUserInfo[i]->m_UserData.dwUserID);
...
void	CServerGameDesk::SendGameDataEx(BYTE bDeskStation, void * pData, UINT uSize, BYTE bMainID, BYTE bAssID, BYTE bHandleCode)
{
	...
	{
		if (bAssID == ASS_SEND_ALL_CARD)
		{
			SendAllStruct* pAllCard = (SendAllStruct*)pData;
			//发送数据
			int iTempPos = 0 ;
			for(int  j  = 0  ; j < PLAY_COUNT  ;j++)
			{
				::CopyMemory(&pAllCard->iUserCardList[iTempPos],m_iUserCard[j],sizeof(BYTE)*m_iUserCardCount[j]);
		...
		case GS_WAIT_ARGEE:			//等待玩家开始状态
			{
				...
				pstation = &GameStation;
				iSize = sizeof(GameStation);
				break;
			}
		case GS_SEND_CARD:		//发牌状态
		case GS_WAIT_BACK:		//等待埋底牌状态
...
{
	SendGameDataEx(nDeskStation,NULL , 0, MDM_GM_GAME_NOTIFY,bAssID,0) ; 
}

//通知游戏可更新SN
void CServerGameDesk::SetGameSN()
{
	//CreateNewRecord();
	return;
};
...
bool	CServerGameDesk::GameBegin(BYTE bBeginFlag)
{
	...
	::ZeroMemory(m_iLastOutCard, 0, sizeof(m_iLastOutCard));
	m_bIsLastCard = false;
	m_byteHitPass = 0;
	...
	{
		CString str;
		str.Format("chenlog----GameBegin-[%s]  %d",m_pUserInfo[i]->m_UserData.nickName,m_GameMutiple.sMingPaiMutiple[i] );
		OutputDebugStringA(str);
	}

	for (int i = 0; i < PLAY_COUNT; i ++)
	{
		SendGameDataEx(i,&TGameBegin,sizeof(TGameBegin),MDM_GM_GAME_NOTIFY,ASS_GAME_BEGIN,0);
	}
...
BOOL	CServerGameDesk::GamePrepare()
{
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
	{
		SendGameDataEx(i,&TGamePrepare,sizeof(TGamePrepare),MDM_GM_GAME_NOTIFY,ASS_GAME_PREPARE,0);
	}
...
BOOL CServerGameDesk::SendCardMsg(BYTE bDeskStation,BYTE bCard)
{
	...
	if(bCard == m_bThrowoutCard)
	{
		...
		for(int i = 0; i < PLAY_COUNT; i ++)
			SendGameDataEx(i,&sendcard,sizeof(sendcard),MDM_GM_GAME_NOTIFY,ASS_SEND_CARD_MSG,0);
...
unsigned char CServerGameDesk::GetCardValue(const CString &cardListStr, int cnt)
{
...
BOOL	CServerGameDesk::SendCardFinish()
{
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
	{
		...
		SendGameDataEx(i,&Tsendcardfinish,sizeof(Tsendcardfinish),MDM_GM_GAME_NOTIFY,ASS_SEND_FINISH,0);
	}
	...
	else if(0 == m_iModel)//抢地主模式
	{
		...
		CString llog;
		llog.Format("lbtest::::::::::m_iCurrentRobPeople = %d",m_iCurrentRobPeople);
...
BOOL CServerGameDesk::SendCallScore(BYTE bDeskStation)
{
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
		SendGameDataEx(i,&CallScore,sizeof(CallScore),MDM_GM_GAME_NOTIFY,ASS_CALL_SCORE,0);
	...
	SetTimer(TIME_JIAO_FEN, (m_iCallScoreTime+1) * 1000); 
...
BOOL CServerGameDesk::UserCallScore(BYTE bDeskStation, int iVal)
{
	...
	else
	{
		...
		if( iVal == 3)						//有人直接叫3分
		{
			...
			for(int i = 0; i < PLAY_COUNT; i ++)
				SendGameDataEx(i,&callscore,sizeof(callscore),MDM_GM_GAME_NOTIFY,ASS_CALL_SCORE_RESULT,0);		//将叫分情况发给其他用户
...
BOOL CServerGameDesk::CallScoreFinish()
{
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
	{
		SendGameDataEx(i,&scoreresult,sizeof(scoreresult),MDM_GM_GAME_NOTIFY,ASS_CALL_SCORE_FINISH,0);		//将叫分情况发给其他用户
	}
...
BOOL	CServerGameDesk::SendRobNT(BYTE bDeskStation)
{
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
	{
		SendGameDataEx(i,&TRobNT,sizeof(TRobNT),MDM_GM_GAME_NOTIFY,ASS_ROB_NT,0);		
	}
	SendWatchData(m_bMaxPeople,&TRobNT,sizeof(TRobNT),MDM_GM_GAME_NOTIFY,ASS_ROB_NT,0);

	SetTimer(TIME_ROB_NT, (m_iRobNTTime+1) * 1000); 
	return TRUE;
}
/*------------------------------------------------------------------------------*/
//玩家抢地主消息
BOOL	CServerGameDesk::UserRobNT(BYTE bDeskStation, int iValue)
{
	...
	{
		CString llog;
		llog.Format("lbtest:::::::::bDeskStation = %d,:m_iCurrentRobPeople = %d",bDeskStation,m_iCurrentRobPeople);
		OutputDebugString(llog);
		return TRUE ; 
	}
	...
	for(int i = 0;i < PLAY_COUNT; i ++)
	{
		SendGameDataEx(i,&robnt,sizeof(robnt),MDM_GM_GAME_NOTIFY,ASS_ROB_NT_RESULT,0);		  //将叫分情况发给其他用户
	}
...
BOOL	CServerGameDesk::RobNTFinish()
{
	...
	for (int i = 0; i < PLAY_COUNT; i ++) 
	{
		SendGameDataEx(i,&robresult,sizeof(robresult),MDM_GM_GAME_NOTIFY,ASS_ROB_NT_FINISH,0);
	}
...
BOOL	CServerGameDesk::SendBackCard()
{
	...
	for (int i = 0; i < PLAY_COUNT; i ++) 
	{
		SendGameDataEx(i,&BackCard,sizeof(BackCard),MDM_GM_GAME_NOTIFY,ASS_BACK_CARD_EX,0);
	}
...
BOOL	CServerGameDesk::SendAddDouble()
{
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
	{
		...
		SendGameDataEx(i,&adddouble,sizeof(adddouble),MDM_GM_GAME_NOTIFY,ASS_ADD_DOUBLE,0);		//将叫分情况发给其他用户
	}
...
BOOL	CServerGameDesk::UserAddDouble(BYTE bDeskStation, int iVal)
{
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
	{
		SendGameDataEx(i,&adddouble,sizeof(adddouble),MDM_GM_GAME_NOTIFY,ASS_ADD_DOUBLE_RESULT,0);		//将叫分情况发给其他用户
	}
...
BOOL	CServerGameDesk::AddDoubleFinish()
{
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
	{
		SendGameDataEx(i,&adddouble,sizeof(adddouble),MDM_GM_GAME_NOTIFY,ASS_ADD_DOUBLE_FINISH,0);		//将叫分情况发给其他用户
	}
...
BOOL	CServerGameDesk::SendShowCard()
{
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
	{
		SendGameDataEx(i,&show,sizeof(show),MDM_GM_GAME_NOTIFY,ASS_SHOW_CARD,0);		//将叫分情况发给其他用户
	}
...
BOOL	CServerGameDesk::UserShowCard(BYTE bDeskStation,int iValue)
{
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
	{
		SendGameDataEx(i,&showresult,sizeof(showresult),MDM_GM_GAME_NOTIFY,ASS_SHOW_CARD_RESULT,0);		//将叫分情况发给其他用户
	}
...
BOOL	CServerGameDesk::ShowCardFinish()
{
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
	{
		SendGameDataEx(i,&showresult,sizeof(showresult),MDM_GM_GAME_NOTIFY,ASS_SHOW_CARD_FINISH,0);		//将叫分情况发给其他用户
	}
...
BOOL	CServerGameDesk::BeginPlay()
{
	...
	for (int i = 0;i < PLAY_COUNT; i ++)
	{
		SendGameDataEx(i,&Begin,sizeof(Begin),MDM_GM_GAME_NOTIFY,ASS_GAME_PLAY,0);
	}
...
//记录出牌
void CServerGameDesk::SetRecorder(BYTE bDeskStation,  BYTE iOutCard[], int iCardCount)
{
	//查找插入位置
	int iInsertPos = 0;
	for(int i = 54 * 2 - 1; i >= 0 ; i--)
	{
		if(i == 0)
		{
			iInsertPos = 1;
		...
		if(m_tOutCardRecorder.byOurCards[bDeskStation][i] != 0 )
		{
			iInsertPos = i + 2;
			break;
		}
	...
	if(iCardCount == 0)
	{
		m_tOutCardRecorder.byOurCards[bDeskStation][iInsertPos] = 255;
	}
	else
	{
		for(int j = 0; j < iCardCount; j++)
		{
			m_tOutCardRecorder.byOurCards[bDeskStation][iInsertPos + j] = iOutCard[j];
			//处理剩余牌
			m_tOutCardRecorder.byLeftCard[iOutCard[j] % 16] --;
		}
	}
}
...
BOOL	CServerGameDesk::UserOutCard(BYTE bDeskStation,  BYTE iOutCard[],  int iCardCount)
{
	...
	//if(iCardCount != m_Logic.GetCardCount(iOutCard,45))
	//{
	//	OutputDebugString("llog::::::::出牌数据不对");
	//	return true;
	//}
	...
	//记录出牌
	SetRecorder(bDeskStation, iOutCard, iCardCount);
	...
	OutCardMsg2AI UserOutResultAI;
	UserOutResultAI.outCard = UserOutResult;
	memcpy_s(UserOutResultAI.bHandCard,sizeof(UserOutResultAI.bHandCard),m_iUserCard[bDeskStation],m_iUserCardCount[bDeskStation]);
	UserOutResultAI.iHandCardCount = m_iUserCardCount[bDeskStation];
	...
	for (int i = 0;i < PLAY_COUNT; i ++) 
	{
		...
		if(m_pUserInfo[i]->m_UserData.isVirtual)
			SendGameData(i,&UserOutResultAI,sizeof(OutCardMsg2AI),MDM_GM_GAME_NOTIFY,ASS_OUT_CARD_RESULT,0);
		else
			SendGameDataEx(i,&UserOutResult,sizeof(OutCardMsg),MDM_GM_GAME_NOTIFY,ASS_OUT_CARD_RESULT,0);
	}
	...
	{
		//m_TCalculateBoard[bDeskStation].iBombCount++;

		m_CulCalcInf[bDeskStation].iBombCount++;
	}
	...
	{
		SetTimer(TIME_OUT_CARD_DELAY, 1000);
	}
	else
	{
		UserOutCardNotify();
...
BOOL CServerGameDesk::UserOutCardNotify( void )
{
	OutCardMsg UserOut;
	...
	{
		SendGameDataEx(i,&UserOut,sizeof(OutCardMsg),MDM_GM_GAME_NOTIFY,ASS_OUT_CARD,0);
	}
...
BOOL CServerGameDesk::IsNewTurn()
{
	...
	{
		for(int i = 0; i < PLAY_COUNT; i ++)
			SendGameDataEx(i,MDM_GM_GAME_NOTIFY,ASS_ONE_TURN_OVER,0);
...
BOOL CServerGameDesk::NewPlayTurn(BYTE bDeskStation)
{
	...
	for (int i = 0; i < PLAY_COUNT; i ++) 
		SendGameDataEx(i,&turn,sizeof(turn),MDM_GM_GAME_NOTIFY,ASS_NEW_TURN,0);
...
BOOL CServerGameDesk::IsAwardPoin(BYTE iOutCard[],int iCardCount,BYTE bDeskStation)
{
	if(m_Logic.IsKingBomb(iOutCard,iCardCount) || m_Logic.IsBomb(iOutCard,iCardCount))
	{
		...
		//发送奖分情况
		for (int i = 0;i < PLAY_COUNT; i++) 
			SendGameDataEx(i,&award,sizeof(award),MDM_GM_GAME_NOTIFY,ASS_AWARD_POINT,0);
		SendWatchData(m_bMaxPeople,&award,sizeof(award),MDM_GM_GAME_NOTIFY,ASS_AWARD_POINT,0);
	}
	return true;
}

bool CServerGameDesk::cul2TotalInf()
{
	for (int i = 0;i < PLAY_COUNT;i++)
	{
		m_TCalculateBoard[i].iBombCount += m_CulCalcInf[i].iBombCount;
	}
	return true;
}

void CServerGameDesk::UpdateCalculateBoard(BYTE bCloseFlag)
{
	if (m_bIsBuy)
	{
		if(((m_iRunGameCount>0 || IsPlayGame(0))&& GFF_DISSMISS_FINISH == bCloseFlag)
			|| (m_iRunGameCount >= m_iVipGameCount))
		{
			...
			for (int i = 0; i < PLAY_COUNT; i++)
			{
				...
				if (nullptr != m_pUserInfo[i])
				{
					m_TCalculateBoard[i].i64UserMoney = m_pUserInfo[i]->m_UserData.i64Money;
				}		
			...
			{	
				for (int i = 0; i < PLAY_COUNT; ++i)
				{
					if ( m_TCalculateBoard[i].i64WinMoney == m_TCalculateBoard[iBigWinnerStation].i64WinMoney )
					{
						m_TCalculateBoard[i].bWinner = true;
					}
				}
			}
			for (int i = 0; i < PLAY_COUNT; i++)
...
// 			}
			...
			if(m_pDataManage->m_InitData.uComType != 1 ) 
			{
				...
				for(int i = 0 ; i< PLAY_COUNT ; i++)
				{
					GameEnd.iTurePoint[i] *= m_iBasePoint ; 
				}
			...
			for (int i = 0; i < PLAY_COUNT; i++)
			{
				m_TCalculateBoard[i].i64WinMoney += GameEnd.iTurePoint[i];
			}
			...
			//继续
			if(m_iBeenPlayGame >= m_iLimitPlayGame && !(m_pDataManage->m_InitData.dwRoomRule & GRR_CONTEST) && !(m_pDataManage->m_InitData.dwRoomRule & GRR_TIMINGCONTEST))//本轮结束
			{
				m_bGameStation = GS_WAIT_ARGEE;

				for (int i = 0; i < PLAY_COUNT; i ++) 
					SendGameDataEx(i,&GameEnd,sizeof(GameEnd),MDM_GM_GAME_NOTIFY,ASS_NO_CONTINUE_END,0);
			...
			{
				for (int i = 0; i < PLAY_COUNT; i ++) 
					SendGameDataEx(i,&GameEnd,sizeof(GameEnd),MDM_GM_GAME_NOTIFY,ASS_CONTINUE_END,0);
			...
			if (m_bIsBuy && m_iVipGameCount > m_iRunGameCount)
			{
				_putRoomInfo();
			}
			...
			UpdateCalculateBoard(bCloseFlag);
			ReSetGameState(bCloseFlag);
			__super::GameFinish(bDeskStation,GFF_SAFE_FINISH);
			int iLeaveJuShu = m_iVipGameCount - m_iRunGameCount;
			for (int i = 0; i < PLAY_COUNT; i++)
			{
				if (m_pUserInfo[i] == NULL)
	...
	case GF_NO_CALL_SCORE:
		{
			...
			//发送数据
			for (int i = 0; i < PLAY_COUNT; i ++) 
				SendGameDataEx(i,&GameEnd,sizeof(GameEnd),MDM_GM_GAME_NOTIFY,ASS_NO_CALL_SCORE_END,0);
	...
	case GF_SAFE:			//游戏安全结束
		{
			...
			for (int i = 0; i < PLAY_COUNT; i ++) 
				SendGameDataEx(i,&CutEnd,sizeof(CutEnd),MDM_GM_GAME_NOTIFY,ASS_SAFE_END,0);
	...
	case GFF_FORCE_FINISH:		//用户断线离开
		{
			...
			for (int i = 0; i < PLAY_COUNT; i ++)
				SendGameDataEx(i,&CutEnd,sizeof(CutEnd),MDM_GM_GAME_NOTIFY,ASS_CUT_END,0);
	...
	case GF_AHEAD_END://提前结束
		{
			...
			::memset(&CutEnd,0,sizeof(CutEnd));
			for (int i = 0; i < PLAY_COUNT; i ++)
				SendGameDataEx(i,&CutEnd,sizeof(CutEnd),MDM_GM_GAME_NOTIFY,ASS_AHEAD_END,0);
	...
	case GFF_DISSMISS_FINISH:
		{
			...
			UpdateCalculateBoard(bCloseFlag);
			ReSetGameState(GFF_FORCE_FINISH);
			m_bGameStation = GS_WAIT_NEXT;//GS_WAIT_SETGAME;
			memset(m_bUserReady,0,sizeof(m_bUserReady));
			__super::GameFinish(bDeskStation,bCloseFlag);
			return true;
		}
...
BOOL CServerGameDesk::UserHaveThing(BYTE bDeskStation, char * szMessage)
{
	...
	if (m_iLeaveArgee != 7)
	{
		...
		for (int i = 0; i < PLAY_COUNT; i ++)
			if (i != bDeskStation)
				SendGameDataEx(i,&HaveThing,sizeof(HaveThing),MDM_GM_GAME_NOTIFY,ASS_HAVE_THING,0);
	}
	else 
...
BOOL CServerGameDesk::ArgeeUserLeft(BYTE bDeskStation, BOOL bArgee)
{
	...
	if (m_iLeaveArgee != 7)				//3个人游戏
	{
		...
		for (int i = 0; i < PLAY_COUNT; i ++) 
			if (i != bDeskStation) 
				SendGameDataEx(i,&Leave,sizeof(Leave),MDM_GM_GAME_NOTIFY,ASS_LEFT_RESULT,0);
	}
	else
	{
		GameFinish(255,GF_SAFE);
		...
		SendGameDataEx(m_iHaveThingPeople,&Leave1,sizeof(Leave1),MDM_GM_GAME_NOTIFY,ASS_LEFT_RESULT,0);
	}
	if (m_icountleave>=2)
	{
		m_iLeaveArgee=0;
...
{
	if (!bAutoCard) m_iOutTimeCount[bDeskStation] = 0;
	...
	for(int i = 0; i < PLAY_COUNT; i ++)
		SendGameDataEx(i,&autoset,sizeof(autoset),MDM_GM_GAME_NOTIFY,ASS_AUTO,0);
...
BOOL CServerGameDesk::SetAIMachine(BYTE bDeskStation,BOOL bState)
{
	...
	for(int i = 0; i < PLAY_COUNT ;i++)
	{
		if(i != bDeskStation &&m_pUserInfo[i] != NULL)		
			SendGameDataEx(i,&ai,sizeof(ai),MDM_GM_GAME_NOTIFY,ASS_AI_STATION,0);	
	}
...
bool CServerGameDesk::UserNetCut(BYTE bDeskStation, CGameUserInfo * pLostUserInfo)
{
	...
	for (int i = 0;i<PLAY_COUNT;i++)
	{
		...
		SendGameDataEx(i,&bagUserleft,sizeof(bagUserleft),MDM_GM_GAME_NOTIFY,ASS_USER_LEFTDESK,0);
	}
	UseAI(bDeskStation);
	return true;
}
...
void	CServerGameDesk::SpuerExamine(BYTE byDeskStation)
{
	...
	if (IsSuperUser(byDeskStation))
	{
		...
		SendGameDataEx(byDeskStation,&TSuperUser,sizeof(TSuperUser),MDM_GM_GAME_NOTIFY,ASS_SUPER_USER,0);
	}
}
//---------------------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------------------
...
//通知游戏更新桌子信息
void CServerGameDesk::CanGetDeskInfo()
{
#ifdef _CREATEROOM_PARA_JSON
	parse(m_szDeskConfig);
#endif
...
//************************************
// Method:    readFromFile
// FullName:  CServerGameDesk::readFromFile
// Access:    public 
// Returns:   bool
// Qualifier: 读文件
// Parameter: const string & filename
// Parameter: string & strbuf
//************************************
bool CServerGameDesk::readFromFile(const string& filename,string & strbuf)
{
	ifstream    in;
...
//************************************
// Method:    parse
// FullName:  CServerGameDesk::parse
// Access:    public 
// Returns:   bool
// Qualifier: 解析创建房间的配置项json格式
// Parameter: const string & strSrc
//************************************
bool CServerGameDesk::parse(const string& strSrc)
{
	using rapidjson::Document;
	Document doc;
	doc.Parse<0>(strSrc.c_str());
	...
	*/
	...
	if (va.IsArray())
	{
		for(size_t i = 0;i < va.Size();i++)
		{
			Value & v = va[i];
			assert(v.IsObject());
			if (v.HasMember("firstName") && v["firstName"].IsString())
			{
				cout<<v["firstName"].GetString()<<endl;
			}
			if (v.HasMember("lastName") && v["lastName"].IsString())
			{
				cout<<v["lastName"].GetString()<<endl;
			}
		}
	}
#endif
...
void CServerGameDesk::_putRoomInfo()
{
	...
	for (int i = 0;i < PLAY_COUNT;i++)
	{
		...
		inf[i].iBombCount = m_TCalculateBoard[i].iBombCount;
		inf[i].iWinMoney = m_TCalculateBoard[i].i64WinMoney;
		for(int j = 0; j < m_vecWinInfo[i].GetCount() && j < MAX_GAME_TURNS;j++)
		{
			inf[i].uWinInfo[j]  = m_vecWinInfo[i].GetAt(j);
		}
...
void CServerGameDesk::SetReturnGameInfo(int UserID,void* szUserGameInfo,int iSize,int iCount)
{
	...
	for (int i = 0;i < PLAY_COUNT;i++)
	{
		...
		{
			m_TCalculateBoard[i].iBombCount = pGameInf->iBombCount;
			m_TCalculateBoard[i].i64WinMoney = pGameInf->iWinMoney;

			DebugPrintf(m_pDataManage->m_InitData.uRoomID, "【炸弹个数问题】pGameInf->iBombCount=%d", pGameInf->iBombCount);

			for(int j = 0; j < MAX_GAME_TURNS;j++)
			{
				if (255 == pGameInf->uWinInfo[j])
				{
					break;
				}
				m_vecWinInfo[i].Add(pGameInf->uWinInfo[j]);
			}
		}	
	}
}
