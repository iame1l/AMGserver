...
//判断手牌是否包含
bool isCardContain(BYTE iOutCard[], int iOutCount ,BYTE iHandCard[], int iHandCount)
{
	if (iOutCount > iHandCount)
	{
		return false;
	}
	int iRecount=0;
								   ...
								   bool bFirstOut)
{
	...
	//简单的牌值验证，主要针对机器人出牌  客户端自己做了验证
	if(isCardContain(iOutCard,iOutCount,iHandCard,iHandCount))
	{
		return TRUE;
	}
	else
