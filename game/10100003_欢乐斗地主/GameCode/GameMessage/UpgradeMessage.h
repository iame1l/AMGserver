...
//用户出牌数据包 （发向AI）
struct OutCardMsg2AI
{
	OutCardMsg			outCard;
	BYTE				bHandCard[45];					//玩家手牌
	int					iHandCardCount;	
	OutCardMsg2AI()
	{
		memset(this,0,sizeof(OutCardMsg2AI));
	}
};
...
//游戏状态数据包	（ 等待下盘开始状态  ）
struct GameStation_5 : public GameStation_Base
{
	BYTE				iBeginTime;							//开始准备时间
	BYTE				iThinkTime;							//出牌思考时间
	BYTE                iRobNTTime ;                    //抢地主时间
...
//当局统计信息（用于大结算）
struct CurCalcBoardInf
{
	int		iBombCount;				//炸弹次数
	CurCalcBoardInf()
	{
		memset(this,0,sizeof(CurCalcBoardInf));
	}
};
...
{
	BYTE uWinInfo[MAX_GAME_TURNS];			//输赢记录
	int iBombCount;							//炸弹次数
	__int64 iWinMoney;						//输赢金币
	GameInfForReplay()
	{
		memset(uWinInfo,255,sizeof(uWinInfo));
		iBombCount = 0;
		iWinMoney = 0;
	}
};

struct TOutCardRecorder
{
	BYTE		byLeftCard[16];						//剩余牌数
	BYTE		byOurCards[PLAY_COUNT][54*2];		//出过的牌(过牌会放入255，每次出牌会用0隔开。举例：22 0 88 0 255 0 ……)
	BYTE		byNtUser;							//庄家，最先出牌玩家
	TOutCardRecorder()
	{
		InitObject();	
	}
...
struct T_S_C_RECORDER_RSP
{
	TOutCardRecorder	tOutCardRecorder;
	T_S_C_RECORDER_RSP()
	{
...
#define S_C_USER_SIT                    100             //玩家坐下

#define C_S_RECORDER_REQ				101				//查看记牌器请求（空结构体）
#define S_C_RECORDER_RSP				102				//记牌器回复（T_S_C_RECORDER_RSP）

#define ASS_BEGIN_TIME					103				//
