...
#define SUB_S_CHART_MSG                     115//聊天
#define S_C_BULLET_PERAMETER_SIG            116//子弹发射频率
...
struct S_C_SUPER_CONFIG
{
	...
	S_C_SUPER_CONFIG()
	{
		memset(this,0,sizeof(S_C_SUPER_CONFIG));
	}
};
...
struct S_C_FISH_NAME
{
	...
	S_C_FISH_NAME()
	{
		memset(this,0,sizeof(S_C_FISH_NAME));
	}
};
...
struct CMD_S_GameStatus {
  ...
  SCORE user_leave_score[GAME_PLAYER];
};
...
struct CMD_S_GameConfig {
  ...
  //int fish18_max_mutiple;
  //int fish19_max_mutiple;
  //int fishLK_max_mutiple;

  int fish_min_multiple[FISH_KIND_COUNT];
  int fish_max_multiple[FISH_KIND_COUNT];
...
struct S_C_BulletPerameter
{
	int iFireFrequency;
	S_C_BulletPerameter()
	{
		memset(this,0,sizeof(S_C_BulletPerameter));
	}
};
...
#define C_S_UPDATE_SUPER_INFO_SIG			18
