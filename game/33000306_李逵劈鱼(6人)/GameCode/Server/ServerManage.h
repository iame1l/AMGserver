...
public:
	...
	bool OnSubUserFire(BYTE bDeskStation, BulletKind bullet_kind, float angle, int bullet_mul, int lock_fishid,int iBullet_id);
...
public:
	...
	__int64 GetRandScore(__int64 iMoney); //主要是用于重连时，机器人随意获取分数
...
private:
	...
	int				fish_max_multiple_[FISH_KIND_COUNT];//各种鱼的最大分数
	...
	int m_iFireFrequency;
	int m_iCellScore;//单次增加的金币数目
