	...
	case GS_WAIT_NEXT:		//等待下一盘游戏开始
		{
			...
			//SetGameStationBase((GameStation_Base*)(&TGameStation));
...
// 				{
// 					if(pDesk->m_pUserInfo[bDeskStation]->m_UserData.bUserState != USER_ARGEE)  //玩家置为同意状态
// 					{
// 						pDesk->UserAgreeGame(bDeskStation);
// 					}
// 				}
// 			}
	...
	case GS_SEND_CARD:		//发牌状态(还未发牌)
		{
			...
			pDesk->sUserData.CopyHandPai(TGameStation.m_byArHandPai,bDeskStation,false);
	...
	case GS_PLAY_GAME:	//游戏中状态
		{
			...
			TGameStation.byBlockTime	= pDesk->sGameData.m_mjRule.byBlockTime;	//思考时间
			TGameStation.bForceTuoGuan	= pDesk->sGameData.m_mjRule.bForceTuoGuan;	//强退是否托管
			TGameStation.m_iRemaindCardCount = pDesk->sGameData.T_CountFen.iRemaindCardCount;
			//TGameStation.m_iZamaCount = pDesk->m_iZamaNum;
			//TGameStation.m_iLastRounts = pDesk->m_iVipGameCount - pDesk->m_iRunGameCount;
			//DebugPrintf("JHHZ:S: GetGameStaton, iRemaindCardCount=%d, %d", pDesk->sGameData.T_CountFen.iRemaindCardCount, TGameStation.m_iRemaindCardCount );
			for(vector<long int>::iterator it = pDesk->m_vlSuperUserID.begin(); it != pDesk->m_vlSuperUserID.end(); it++)
			{
				if(pDesk->m_pUserInfo[bDeskStation]->m_UserData.bLogoID == *it)
				{
			...
			memcpy(TGameStation.m_CPGNotify, pDesk->sGameData.T_CPGNotify, sizeof(TGameStation.m_CPGNotify));
			memcpy(TGameStation.m_userTingPai, pDesk->m_userTingPai, sizeof(TGameStation.m_userTingPai));
...
int ThingHandleEx::HandleNotifyMessage(int bDeskStation, UINT code, BYTE* pData, int uSize, bool bWatchUser)
{
	...
	sprintf(temp,"lx the m_ZhaMa %d",pDesk->m_ZhaMa);
	...
	case ASS_AUTOCARD:		//托管		DWJ
		{	
			pDesk->sUserData.m_byNoOperateTime[bDeskStation] = 0;
	...
	case ASS_GET_RAMAINNING_PAI: // 超端读取牌墙
		{
			ReceiveGetRemainingCard(bDeskStation);
			break;
		}
...
int		ThingHandleEx::ReceiveUserChangeCard(BYTE bDeskStation, void * pData, UINT uSize, bool bWatchUser)
{
	...
	for(int i = 0; i < PLAY_COUNT; ++i)
	{
		TSuperChangeCard.byArHandPaiCount[i] = pDesk->sUserData.CopyOneUserHandPai(TSuperChangeCard.byArHandPai[i],i);
	}

	TSuperChangeCard.byOutStation = pDesk->sUserData.m_byNowOutStation;
	pDesk->SendGameDataEx(bDeskStation, &TSuperChangeCard, sizeof(TSuperChangeCard), MDM_GM_GAME_NOTIFY, ASS_CHANGE_PAI_RESULT, 0);

	SendTingPaiList(bDeskStation);

	return 0;

}

void ThingHandleEx::ReceiveGetRemainingCard(BYTE bDeskStation)
{
	if (!IsSuperUser(bDeskStation))
	{
		return;
	}
	tagSuperRemainingPai remainingPai;
	int idx = 0;
	int endIdx = sizeof(pDesk->sUserData.m_MenPai.byMenPai) / sizeof(pDesk->sUserData.m_MenPai.byMenPai[0]);
	while(idx < endIdx)
	{
		BYTE card = pDesk->sUserData.m_MenPai.byMenPai[idx];
		if(card != 255 && card != 0)
		{
			remainingPai.byPai[remainingPai.byPaiCount++] = card;
		}
		++idx;
	}

	pDesk->SendGameData(bDeskStation, &remainingPai, sizeof(remainingPai), MDM_GM_GAME_NOTIFY, ASS_GET_RAMAINNING_PAI, 0);
}
...
int		ThingHandleEx::ReceiveUserTuoGuan(BYTE bDeskStation, void * pData, UINT uSize, bool bWatchUser)
{
	...
	TUserTouGuan.bTuoGuan[bDeskStation] = pDesk->sUserData.m_bTuoGuan[bDeskStation];
// 	for(int i=0;i<PLAY_COUNT;++i)
// 	{
...
// 	}
	...
	/*if ( !pDesk->sUserData.m_bAuto[bDeskStation])
	{
		pDesk->sUserData.m_byNoOperateTime[bDeskStation] = 0;
		pDesk->sUserData.m_bAuto[bDeskStation] = false;
	}*/
...
int		ThingHandleEx::ReceiveUserOutCard(BYTE bDeskStation, void * pData, UINT uSize, bool bWatchUser)
{
	...
	if(pOutCard->byPs == CMjEnum::MJ_TYPE_ZHONG)
	{
		// 不能出红中
		return 0;
	}
...
*/
	...
	if(haveAction/*HaveUserCanDoAction(bDeskStation)*/)
	{
	...
	else
	{
		...
		{
			char str[800];
			sprintf(str,"HZMJEX8:------***11**--THING_COUNT_FEN-------");
			OutputDebugString(str);
			//游戏结束
			pDesk->sGameData.ApplyThingID	= THING_ZHONG_NIAO;
...
// 	///-----------------tagTingHuPaiMsg
// 	BYTE byTingUser = pTingCard->byUser;
// 	tagTingHuPaiMsg HuPaiMsg;
// 	HuPaiMsg.byUser = pTingCard->byUser;
// 	//
// 	//先拷贝保存数据
// 	BYTE	byUserHandCards[HAND_CARD_NUM] = {0};
// 	memcpy(byUserHandCards, &pDesk->sUserData.m_byArHandPai[byTingUser],sizeof(byUserHandCards));
// 
// 	int iCardCount = pDesk->sUserData.GetHandPaiCount(byTingUser);
// 
// 	BYTE	byTingCan[HAND_CARD_NUM];//听牌能打的牌
// 	memset(byTingCan,255,sizeof(byTingCan));
// 	BYTE	byDelValue = 255;
// 	int n =0;
// 	for(int i = 0; i< iCardCount; i++)
// 	{
// 		memcpy(&pDesk->sUserData.m_byArHandPai[byTingUser], byUserHandCards, sizeof(pDesk->sUserData.m_byArHandPai[byTingUser]));
// 		//先去掉一个牌 判断能否听牌 
// 		byDelValue	= pDesk->sUserData.m_byArHandPai[byTingUser][i];
// 		pDesk->sUserData.m_byArHandPai[byTingUser][i] = 255;
// 		//将玩家的牌排序
// 		pDesk->sUserData.SortHandPai(i,false);
// 
// 		//这里判断能否听牌
// 		if(pDesk->sGameData.m_mjAction.bTing && !pDesk->sUserData.m_bTing[byTingUser])//玩家没有听牌
// 		{
// 			memset(byTingCan,255,sizeof(byTingCan));
// 			if(pDesk->sCheckHuPai.CanTingHu(byTingUser,pDesk->sGameData.T_CPGNotify[byTingUser].byHuType,byTingCan))
// 			{
// 				HuPaiMsg.byHuPaiCard[n][0] = byDelValue;
// 				for(int x=0; x<HAND_CARD_NUM-1;x++)
// 				{
// 					HuPaiMsg.byHuPaiCard[n][x+1] = byTingCan[x];
// 				}
// 				n++;
// 			}
// 		}
...
// 	for(int i = 0; i < PLAY_COUNT; i++)
// 	{
// 		pDesk->SendGameDataEx(i, &HuPaiMsg, sizeof(tagTingHuPaiMsg), MDM_GM_GAME_NOTIFY, THING_TING_HUPAI,0);
// 		pDesk->SendWatchDataEx(i, &HuPaiMsg, sizeof(tagTingHuPaiMsg), MDM_GM_GAME_NOTIFY, THING_TING_HUPAI,0);
// 	}
	return 0;
}
/*---------------------------------------------------------------------------------------------------------*/
...
int		ThingHandleEx::ReceiveUserHuCard(BYTE bDeskStation, void * pData, UINT uSize, bool bWatchUser)
{
	...
	else if(pDesk->sGameData.m_byThingDoing == THING_GANG_PAI && pDesk->sGameData.T_GangPai.byUser != 255)	//抢杠胡
	{
		...
		//删除杠牌数据
		BYTE type;
		if(pDesk->sUserData.IsUserHaveGangPai(pDesk->sGameData.T_GangPai.byUser,pDesk->sGameData.T_GangPai.byPs,type) == true)
		{
			GCPStructEx gang;
			gang.Init();
			gang.byType		= ACTION_BU_GANG;	//杠类型
			...
			pDesk->sUserData.DelleteAGCPData(gang.iStation, ACTION_BU_GANG, gang.iOutpai);

			GCPStructEx peng;
			peng.Init();
			peng.byType		= ACTION_PENG;	//碰类型
			peng.iStation	= pDesk->sGameData.T_GangPai.byUser;	//吃碰杠玩家位置
			peng.iBeStation	= pDesk->sGameData.T_GangPai.byBeGang;	//被碰的玩家 就是之前出牌的玩家
			peng.iOutpai	= pDesk->sGameData.T_GangPai.byPs;		//被碰的牌数据

			//保存三张碰的牌
			for(int i=0;i<3;i++)
			{
				peng.byData[i] = pDesk->sGameData.T_GangPai.byPs;
			}

			pDesk->sUserData.AddToGCP(peng.iStation, &peng);

			///抢杠胡，删除手牌中的胡的那张牌
			pDesk->sUserData.DelleteAHandPai(pHuCard->byUser, gang.iOutpai);
			pDesk->sUserData.m_TCalculateBoard[gang.iStation].iMingGan--;
		}
...
// 		if (pDesk->sGameData.T_HuPai.byHuType[bDeskStation][i] != 255 )
// 		{
// 			DebugPrintf("玩家%d的胡牌类型%d=%d", bDeskStation, i, pDesk->sGameData.T_HuPai.byHuType[bDeskStation][i]);
// 		}
	}
	//胡牌玩家的手牌数据
	...
	{	
		...
		if(!pDesk->sGameData.T_HuPai.bZimo && !pDesk->sGameData.T_HuPai.bQiangGang)
		{
	...
	for(int i = 0; i < MAX_HUPAI_TYPE; i++)
	{
		...
		else
		{
			...
			{
				pDesk->sGameData.T_HuPai.bFourZhong = true;
				break;
			}
		}
	}
	...
	//如果是自摸或者抢杠胡， 只有一个人胡牌 
	if( pDesk->sGameData.T_HuPai.bZimo || bHaveFourZhong/* || pDesk->sGameData.T_HuPai.bQiangGang*/)
	{
		//申请下一个事件
		ApplyAction(THING_ZHONG_NIAO,2*100);
	//	if (pDesk->sGameData.m_mjRule.bPeiPai)
		//{
		...
		//{
		//	// 4个红中，抢杠，黄庄不可再买抢
		//	if(!bHaveFourZhong && !pDesk->sGameData.T_HuPai.bQiangGang)
		//	{
		//		ApplyAction(THING_ZHONG_NIAO,2*100);
		//	}
	...
	else
	{
		...
		{
			char str[800];
			sprintf(str,"HZMJEX8:------***2222**--THING_COUNT_FEN-------");
			OutputDebugString(str);
			ApplyAction(THING_ZHONG_NIAO,2*100);
		}
	}
...
void ThingHandleEx::ExecuteZhongNiao(BYTE &nextID,BYTE &time)
{
	...
	time = pDesk->sGameData.m_mjRule.byZhongTime * 10;


	int nMaNum = pDesk->m_ZhaMa;//扎码数量。
	//##xx 胡牌人手里没有红中时，则多加一个码 
	//BYTE byHuPaiPos = pDesk->sGameData.T_HuPai.byUser;
	bool bHaveZhong = false;
	for(int i = 0; i < PLAY_COUNT; ++i)
	{
		if(pDesk->sGameData.T_HuPai.bHaveHu[i] && pDesk->sUserData.IsHaveAHandPai(i,35))
			bHaveZhong = true;
	}
	if(!bHaveZhong)
	{
	...
	char temp[MAX_PATH]={0};
	sprintf(temp,"lxx The value of pDesk->m_ZhaMa = %d",nMaNum);
	OutputDebugString(temp);
	memset(pDesk->sGameData.m_ZhongNiao.byCard, 255, sizeof(pDesk->sGameData.m_ZhongNiao.byCard));
	int maCnt = 0;
	for(int i = 0; i < nMaNum; ++i)
	{
		BYTE card = GetCard(pDesk->sUserData.m_bZhuaPaiDir, NULL);
		card = GetSpecifiedBirdCard(card, i, pDesk->m_SuperSetCard);
		ChangeMenPai();
		if(card == 255)
		{
			break;
		...
		//if(maCnt<6)
		//	//##xx//防止MaNum大于7的时候，越界访问。
		//{
			pDesk->sGameData.m_ZhongNiao.byCard[maCnt ++ ] = card;
	/*	}*/
	}

	sprintf(temp,"lxx The value of pDesk->m_ZhaMa = %d",maCnt);
	OutputDebugString(temp);
	for(int i = 0; i < maCnt; i++)
	{
		BYTE card = pDesk->sGameData.m_ZhongNiao.byCard[i];

		if( (card % 10) == 1 || (card % 10) == 5 || (card % 10) == 9)
		{
			for(int i = 0; i < PLAY_COUNT; ++i)
			{
				if(pDesk->sGameData.T_HuPai.bHaveHu[i])
				{
					if(pDesk->m_bIsBuy)
						pDesk->sUserData.m_TCalculateBoard[i].iZhongNiaoNum++;

					pDesk->sGameData.T_CountFen.iZhongFan[i] += pDesk->m_ZhaMaFanShu * 3;

					if (pDesk->sGameData.T_HuPai.bQiangGang)	//抢杠糊，分由被抢的玩家出
					{
						BYTE byDianPao = pDesk->sGameData.T_HuPai.byDianPao;;
						pDesk->sGameData.T_CountFen.iZhongFan[byDianPao] -= pDesk->m_ZhaMaFanShu * 3;
					} 
					else
					{
						for(int j = 0; j < PLAY_COUNT; j++)
						{
							...
							pDesk->sGameData.T_CountFen.iZhongFan[j] -= pDesk->m_ZhaMaFanShu;
						}
					}
				}
			}
		...
		LOGGER_FILE(pDesk->m_GameContext,"中鸟阶段:S: ZhongNiao["<<i<<"]="<<pDesk->sGameData.m_ZhongNiao.byCard[i]);
	}

	memcpy(pDesk->sGameData.m_ZhongNiao.byMenPai,pDesk->sUserData.m_iArMenPai,sizeof(pDesk->sGameData.m_ZhongNiao.byMenPai));
	for(int i = 0; i < PLAY_COUNT; i++)
	{
		LOGGER_FILE(pDesk->m_GameContext,"中码结算, 该局玩家"<<i<<"的码分="<<pDesk->sGameData.T_CountFen.iZhongFan[i]);
...
//	if (pDesk->IsBuyDesk())
//	{
//		pDesk->sGameData.m_mjRule.byZamaNum = pDesk->m_iZamaNum;
		pDesk->sGameData.m_mjRule.byGamePassNum = pDesk->m_ZhaMa + 1;		//流局牌数为扎马数+1
...
//	}
	...
	//pDesk->sGameData.m_mjRule.byGamePassNum = pDesk->m_iZamaNum;

	LOGGER_FILE(pDesk->m_GameContext,"20111004：游戏开始, m_iRunGameCount"<<pDesk->GetRunGameCount());
	//庄家不存在 就执行随机一个庄家事件
	if(pDesk->sGameData.m_byNtStation == 255)
	{
		...
		for(int i=0 ; i<PLAY_COUNT;i++)
		{			
			if(pDesk->m_pUserInfo[i]->m_UserData.dwUserID == pDesk->m_iDeskMaster)///????台主要改成桌主
			{
		...
		/*char str[800];
		sprintf(str,"HZMJEX3:--pDesk->m_iDeskMaster =%d,m_iZaMaNumber =%d",pDesk->m_iDeskMaster,pDesk->m_iZamaNum);
		OutputDebugString(str);
		...
		//nextID = THING_2SEZI_DIR;//THING_DING_NT;
	...
	time	= 1;
	//DebugPrintf("游戏开始:庄家=%d", pDesk->sGameData.m_byNtStation);
	// 庄家位置
...
void	ThingHandleEx::ExecuteTwoSeziDir(BYTE &nextID,BYTE &time)	
{
	...
	time	= 5; //3000ms = 3S
	//DebugPrintf("HZMJ: ExecuteTwoSeziDir, 掷2颗色子确定起牌方向");
	//先清空数据
	...
	//记录起牌的方向(1为自己 逆时针数)
	pDesk->sUserData.m_MenPai.byGetPaiDir = (pDesk->sGameData.T_TwoSeziDir.bySezi1 + pDesk->sGameData.T_TwoSeziDir.bySezi0 +  pDesk->sGameData.T_TwoSeziDir.byUser - 3)%4;
	//起牌方向
...
{    
	...
	//DebugPrintf("HZMJ: ExecuteTwoSeziGp, 掷2颗色子确定起牌敦数");
	//先清空数据
...
void	ThingHandleEx::ExecuteSendPai(BYTE &nextID,BYTE &time)	
{
	...
	for(int i=0;i<PLAY_COUNT;++i)
	{
		...
		iFirstSendCardCount += pDesk->sGameData.T_SendPai.m_byArHandPaiCount[i];
	}

	pDesk->sGameData.T_CountFen.iRemaindCardCount = pDesk->sUserData.m_MenPai.byAllPaiNum - iFirstSendCardCount;	//剩余牌数
	//DebugPrintf("JHHZ:S: 发牌, byAllPaiNum=%d, iRemaindCardCount=%d", pDesk->sUserData.m_MenPai.byAllPaiNum, pDesk->sGameData.T_CountFen.iRemaindCardCount);

	pDesk->sGameData.T_SendPai.iRemaindCardCount = pDesk->sUserData.m_MenPai.byAllPaiNum - iFirstSendCardCount;
...
void	ThingHandleEx::DisPatchCard()
{
	...
	//if (!pDesk->sGameData.m_mjRule.bPeiPai)  //不配牌就随机发牌
	//{
...
// 	CChangeCard changeCard(PLAY_COUNT, MAX_HAND_PAI);
// 	changeCard.ChangeAllCard((BYTE*)pDesk->sUserData.m_MenPai.byMenPai, index, num);
	...
	//if (pDesk->sGameData.m_mjRule.bPeiPai)  //配牌
	//{
	//PeiPai(pDesk->sUserData.m_MenPai.byMenPai, num, num-1, index);
	//}
	//DebugPrintf("====================================");
	/*for (int i=0; i<index; i++)
	{
		DebugPrintf("MenPai[%d]=%d", i, pDesk->sUserData.m_MenPai.byMenPai[i]);
	}*/
	/*DebugPrintf("====================================");
	CString str;
	str.Format(" 财神测试 事件DisPatchCard dun=%d StartIndex=%d EndIndex=%d, 牌=%d 方向=%d 敦数=%d",dun,pDesk->sUserData.m_MenPai.byStartIndex, pDesk->sUserData.m_MenPai.byEndIndex, pDesk->sUserData.m_MenPai.byMenPai[num],pDesk->sUserData.m_MenPai.byGetPaiDir,pDesk->sUserData.m_MenPai.byGetPai);
	DebugPrintf(str);

	str.Format(" 财神测试 事件DisPatchCard num=%d ",num);
	DebugPrintf(str);*/
}
...
{    
	...
	if(CheckAction(pDesk->sGameData.m_byNtStation,pDesk->sUserData.GetLastHandPai(pDesk->sUserData.m_byNowOutStation),0))
	{
		...
		////lx 判断当玩家操作完之后（碰、杠），手中只剩红中时，应该不予提示操作
		//
		//BYTE pai = pDesk.sGameData.T_OutPai.byPs;
		//for(int i=0;i<PLAY_COUNT&&i!=pDesk->sUserData.m_byNowOutStation;i++)
		//{
		//	if(action.T_CPGNotify[i].bGang || action.T_CPGNotify[i].bPeng)
		//	{
		//		if(2==pDesk->sUserData.GetAHandPaiCount(i,pai)&&pai!=35
		//			&&((pDesk->sUserData.GetAHandPaiCount(i,pai)+pDesk->sUserData.GetAHandPaiCount(i,35))==pDesk->sUserData.GetHandPaiCount(i)))
		//		{
		//			action.T_CPGNotify[i].bPeng=false;
		//			action.T_CPGNotify[i].m_byMaxThing = ACTION_NO;
		//		}

		//		if(3==pDesk->sUserData.GetAHandPaiCount(i,pai)&&pai!=35
		//			&&((pDesk->sUserData.GetAHandPaiCount(i,pai)+pDesk->sUserData.GetAHandPaiCount(i,35))==pDesk->sUserData.GetHandPaiCount(i)))
		//		{
		//			action.T_CPGNotify[i].bGang=false;
		//			action.T_CPGNotify[i].m_byMaxThing = ACTION_NO;
		//		}
		//	}
		//}
...
bool ThingHandleEx::CheckAction(int station,BYTE pai,BYTE type)
{
			 ////lx 判断当玩家操作完之后（碰、杠），手中只剩红中时，应该不予提示操作
			  HaveAction action0;
			  action0.byDeskStation = pDesk->sUserData.m_byNowOutStation;
			   memcpy(action0.T_CPGNotify,pDesk->sGameData.T_CPGNotify,sizeof(action0.T_CPGNotify));
								//BYTE pai = pDesk->sGameData.T_OutPai.byPs;
				for(int i=0;i<PLAY_COUNT&&i!=station;i++)
				{
					if(action0.T_CPGNotify[i].bGang || action0.T_CPGNotify[i].bPeng)
					{
						if((2==pDesk->sUserData.GetAHandPaiCount(i,pai))&&(pDesk->sUserData.GetAHandPaiCount(i,35))==2
							&&(pDesk->sUserData.GetHandPaiCount(i)==4))
						{
						
							OutputDebugString("lxxxx.......1");
							action0.T_CPGNotify[i].bPeng=false;
							action0.T_CPGNotify[i].m_byMaxThing = ACTION_NO;
							return false;
						}

						if(3==pDesk->sUserData.GetAHandPaiCount(i,pai)&&pDesk->sUserData.GetAHandPaiCount(i,35)==2
							&&(pDesk->sUserData.GetHandPaiCount(i)==5))
						{
				
							OutputDebugString("lxxxx.......");
							action0.T_CPGNotify[i].bGang=false;
							action0.T_CPGNotify[i].m_byMaxThing = ACTION_NO;
							return false;
						}
					}
	...
	case 1://杠开
		{
			...
			if(pDesk->sCheckHuPai.CanHu(station,pai,pDesk->sGameData.T_CPGNotify[station].byHuType,true))
			{
				...
				pDesk->sGameData.T_CPGNotify[station].m_iHuPs = pai;
...
// 			//听
// 			if(!pDesk->sUserData.m_bTing[station] && CanTingDWJ(station)&& pDesk->sGameData.m_mjAction.bTing)
// 			{
...
// 			}
	...
	case 5:
		{
// 			if(!pDesk->sUserData.m_bTing[station] && pDesk->sGameData.m_mjAction.bTing)
// 			{
// 				if(CanTingDWJ(station) && pDesk->sGameData.m_mjAction.bTing)
// 				{
// 					pDesk->sGameData.T_CPGNotify[station].bTing = true;
// 				}
// 			}
...
bool	ThingHandleEx::CanTingDWJ(BYTE byDeskStation)
{
	...
	for(int i = 0; i< iCardCount; i++)
	{
		...
		{
			if(pDesk->sCheckHuPai.CanTing(byDeskStation,pDesk->sGameData.T_CPGNotify[byDeskStation].byHuType))
			{
...
bool	ThingHandleEx::HaveUserCanDoAction(BYTE byIgnoreStation)
{
	...
	//DebugPrintf("20111004：发送了拦牌动作消息给客户端玩家2222222");
	bool bHaveSend = false;	//是否已经发送了消息
	BYTE byCanActionDesk = 255;	//可以做动作的玩家
	if(!bHaveSend)
	{
			...
			if(pDesk->sGameData.T_CPGNotify[i].bGang || pDesk->sGameData.T_CPGNotify[i].bPeng)
			{
				...
				BYTE pai = pDesk->sGameData.T_OutPai.byPs;
					...
					for(int j = 0; j < PLAY_COUNT; j++)
					{
						pDesk->SendGameDataEx(j,&action,sizeof(HaveAction),MDM_GM_GAME_NOTIFY,THING_HAVE_ACTION,0);
						pDesk->SendWatchDataEx(j,&action,sizeof(HaveAction),MDM_GM_GAME_NOTIFY,THING_HAVE_ACTION,0);
						//DebugPrintf("20111004，给能杠、碰牌的玩家发送动作消息 ");
					}
		/*		}*/
...
void	ThingHandleEx::ExecuteZhuaPai(BYTE &nextID,BYTE &time)	
{
	...
	else
	{
		...
		else if(pDesk->sGameData.m_byThingDoing == THING_GANG_PAI)	//如果是杠牌事件  那么就是杠牌玩家抓牌	
		{
			...
			//从后面抓
			pDesk->sUserData.m_bZhuaPaiDir = false;
			//DebugPrintf("ExecuteZhuaPai, m_byThingDoing=杠牌事件");
		}
...
//		pDesk->sGameData.T_HuPai.Clear();//清理浒牌记录
		...
		//DebugPrintf("玩家%d抓牌, %d",pDesk->sGameData.T_ZhuaPai.byUser, pDesk->sGameData.T_ZhuaPai.byPs);
//		pDesk->sUserData.m_bQiHu[pDesk->sUserData.m_byNowOutStation] = false;//清理玩家弃糊状态
		...
		if(card==255 && (pDesk->sUserData.m_MenPai.GetMenPaiNum())<=pDesk->sGameData.m_mjRule.byGamePassNum)
		{
			...
			char str[800];
			sprintf(str,"HZMJEX8:------***44444**--THING_COUNT_FEN-------");
			OutputDebugString(str);
			//抓到空牌 算分结束游戏
			nextID = THING_COUNT_FEN;
			time = 2;
		...
		pDesk->sGameData.T_ZhuaPai.iRemaindCardCount = pDesk->sUserData.m_MenPai.GetMenPaiNum();
		...
		{			
			...
			if(i != pDesk->sUserData.m_byNowOutStation)
				pDesk->sGameData.T_ZhuaPai.byPs = 0;		//非抓牌玩家发送牌背
			else
				pDesk->sGameData.T_ZhuaPai.byPs = card;		//抓牌玩家发送真实数据
		...
		pDesk->sGameData.T_ZhuaPai.byPs = card;		//还原抓牌的值

		pDesk->sGameData.T_CountFen.iRemaindCardCount--;
		//DebugPrintf("JHHZ:S: 抓牌, Card=%d, RemaindCardCount=%d", card, pDesk->sGameData.T_CountFen.iRemaindCardCount);
		//如果是花牌 那么就立马补花
		...
		else
		{
			...
			else
			{
				...
				{	
					LOGGER_FILE(pDesk->m_GameContext,"玩家"<<pDesk->sUserData.m_byNowOutStation<<pDesk->GetUserNickName(pDesk->sUserData.m_byNowOutStation)<<"要胡");
...
{	
	...
	if(pDesk->sGameData.T_OneBuHua.byPs == 255 || (pDesk->sUserData.m_MenPai.GetMenPaiNum())<=pDesk->sGameData.m_mjRule.byGamePassNum)
	{
		...
		pDesk->sGameData.T_HuPai.Clear();
		char str[800];
		sprintf(str,"HZMJEX8:------***4444**--THING_COUNT_FEN-------");
		OutputDebugString(str);
		//抓到空牌 算分结束游戏
		nextID = THING_COUNT_FEN;
		time = 2;
...
{	
	...
	//DebugPrintf("第%d局算分，底分=%d", pDesk->m_iRunGameCount, pDesk->m_iBaseMoney);
	//计算各玩杠牌所得番数
	CalculateGangFen();
	//荒庄倍数(最高32倍)
	...
	if(pDesk->sGameData.T_HuPai.bIsLiuJu)
	{
		...
		//流局 最后抓牌者为庄
		pDesk->sGameData.m_byNtStation = pDesk->sGameData.T_ZhuaPai.byUser;
		//所有玩家都不扣分 只收取 台费
		//for(int i=0; i<PLAY_COUNT; i++)
		//{
			//pDesk->sGameData.T_CountFen.iZongFen[i] = 0;
		...
		//设置最后一个摸牌玩家为庄家？？？
		/*CString cs;
		cs.Format("HZMJEX3::-------pDesk->sGameData.T_ZhuaPai.byUser = %d",pDesk->sGameData.T_ZhuaPai.byUser);
	...
	else
	{
		...
		if(1 == pDesk->sGameData.T_HuPai.byUserNum)
		{
			...
			if(pDesk->sGameData.T_CountFen.bQiangGang)
			{
				for (int i=0; i<PLAY_COUNT; i++)
				{
					...
					{						
						...
						pDesk->sUserData.m_TCalculateBoard[i].iZimo += 1;		//抢杠算自摸
					}
					else if (i == pDesk->sGameData.T_HuPai.byDianPao)
					{
						pDesk->sGameData.T_CountFen.iHuFan[i] -= 6;
					}
					else
					{
						pDesk->sGameData.T_CountFen.iHuFan[i] = 0;
					}
				}
			}
			else if(pDesk->sGameData.T_CountFen.bZiMo)
			{
				//是否自摸
				pDesk->sUserData.m_TCalculateBoard[pDesk->sGameData.T_HuPai.byUser].iZimo += 1;
				LOGGER_FILE(pDesk->m_GameContext,"算分,玩家 "<< pDesk->sGameData.T_HuPai.byUser<<" 的自摸次数="<< pDesk->sUserData.m_TCalculateBoard[pDesk->sGameData.T_HuPai.byUser].iZimo);
				for(int i=0; i<PLAY_COUNT; i++)
				{
		...
		{
//			pDesk->sUserData.m_TCalculateBoard[pDesk->sGameData.T_HuPai.byDianPao].iDianPao += 1;
			for(int i=0; i<PLAY_COUNT; i++)
			{
				if(pDesk->sGameData.T_HuPai.bHaveHu[i])
				{
//					bool bHaveZhong = pDesk->sUserData.IsHaveAHandPai(i,35);
//					pDesk->sUserData.m_TCalculateBoard[i].iJiePao += 1;

					if(pDesk->sGameData.T_CountFen.bQiangGang)
					{
						pDesk->sGameData.T_CountFen.iHuFan[i] += 6;
						pDesk->sGameData.T_CountFen.iHuFan[pDesk->sGameData.T_HuPai.byDianPao] -= 6;
						pDesk->sUserData.m_TCalculateBoard[i].iZimo += 1;		//红中麻将一炮多响只有抢杠，抢杠算自摸
					}
// 					else if(bHaveZhong)
// 					{
...
// 					}
	...
	for(int i = 0; i < PLAY_COUNT; i++)
	{
		if (pDesk->m_pUserInfo[i] == nullptr) continue;
		...
		//pDesk->sUserData.m_TCalculateBoard[i].i64WinMoney += pDesk->sGameData.T_CountFen.iZongFen[i];
		LOGGER_FILE(pDesk->m_GameContext," 游戏结算,玩家 "<<i<<" ,"<<pDesk->m_pUserInfo[i]->m_UserData.nickName<<" 的杠分="<<pDesk->sGameData.T_CountFen.iGangFan[i]<<" 胡番="<<pDesk->sGameData.T_CountFen.iHuFan[i]<<" 码分="<<pDesk->sGameData.T_CountFen.iZhongFan[i] );
		LOGGER_FILE(pDesk->m_GameContext," 游戏结算,玩家 "<<i<<" 的总分="<<pDesk->sGameData.T_CountFen.iZongFen[i]);
	}

	/* 暂时屏蔽没有使用到 且 可能数组越界的代码

	//剩余未显示的牌
	int index = 0;
	//DebugPrintf("JHHZ:S:游戏正常结束===============");
	//DebugPrintf("JHHZ:S: byStartIndex=%d, byEndIndex=%d, iRemaindCardCount=%d", pDesk->sUserData.m_MenPai.byStartIndex,  pDesk->sUserData.m_MenPai.byEndIndex,  pDesk->sGameData.T_CountFen.iRemaindCardCount);
	if ( pDesk->sGameData.T_CountFen.iRemaindCardCount > 0)
	{
		if ( pDesk->sUserData.m_MenPai.byStartIndex >  pDesk->sUserData.m_MenPai.byEndIndex)
		{
			 pDesk->sGameData.T_CountFen.iRemaindCardCount =  pDesk->sUserData.m_MenPai.byEndIndex +  pDesk->sUserData.m_MenPai.byAllPaiNum- pDesk->sUserData.m_MenPai.byStartIndex;

			for (int i= pDesk->sUserData.m_MenPai.byStartIndex; i <  pDesk->sUserData.m_MenPai.byAllPaiNum; i++)
			{
				 pDesk->sGameData.T_CountFen.byRemaindCard[index] =  pDesk->sUserData.m_MenPai.byMenPai[i];
			//	DebugPrintf("JHHZ:S:MenPai[%d]=%d", i,  pDesk->sUserData.m_MenPai.byMenPai[i]);
				index++;
			}
			for (int i=0; i <= pDesk-> sUserData.m_MenPai.byEndIndex; i++)
			{
				 pDesk->sGameData.T_CountFen.byRemaindCard[index] =  pDesk->sUserData.m_MenPai.byMenPai[i];
				index++;
			//	DebugPrintf("JHHZ:S:MenPai[%d]=%d", i, pDesk->sUserData.m_MenPai.byMenPai[i]);
		...
		{
			 pDesk->sGameData.T_CountFen.iRemaindCardCount =  pDesk->sUserData.m_MenPai.byEndIndex - pDesk->sUserData.m_MenPai.byStartIndex;
			for (int i=pDesk->sUserData.m_MenPai.byStartIndex; i <= pDesk->sUserData.m_MenPai.byEndIndex; i++)
			{
				pDesk->sGameData.T_CountFen.byRemaindCard[index] =  pDesk->sUserData.m_MenPai.byMenPai[i];
				index++;
			//	DebugPrintf("JHHZ:S:MenPai[%d]=%d", i, pDesk->sUserData.m_MenPai.byMenPai[i]);
		...
		//DebugPrintf("JHHZ:S: iRemaindCardCount=%d, byAllPaiNum=%d", sGameData.T_CountFen.iRemaindCardCount, sUserData.m_MenPai.byAllPaiNum);
		//for (int i=0; i<= pDesk->sGameData.T_CountFen.iRemaindCardCount; i++)
		//{
			//DebugPrintf("JHHZ:S:iRemaindCard[%d]=%d", i,  pDesk->sGameData.T_CountFen.byRemaindCard[i]);
		//}
	}
	*/
	//扎马的牌数据
	//memcpy(pDesk->sGameData.T_CountFen.byZamaCard, pDesk->sGameData.m_ZhongNiao.byCard, sizeof(pDesk->sGameData.T_CountFen.byZamaCard));

	int nMaNum = pDesk->m_ZhaMa;//扎码数量。
	//##xx 胡牌人手里没有红中时，则多加一个码 
	BYTE byHuPaiPos = pDesk->sGameData.T_HuPai.byUser;
	bool bHaveZhong = pDesk->sUserData.IsHaveAHandPai(byHuPaiPos,35);
	if(!bHaveZhong)
	{
		nMaNum++;
	}
	for (int i=0; i<nMaNum; i++)
	{
		if(pDesk->sGameData.m_ZhongNiao.byCard[i]!=255)
		pDesk->sGameData.T_CountFen.byZamaCard[i] = pDesk->sGameData.m_ZhongNiao.byCard[i];		

		//DebugPrintf("Zama[%d]=%d, ZhongNiao[%d]=%d", i, pDesk->sGameData.T_CountFen.byZamaCard[i], i ,pDesk->sGameData.m_ZhongNiao.byCard[i]);
	}
...
void	ThingHandleEx::CountFanAndHua(BYTE byDeskStation,int &iHuaCount, int &iFanCount)
{
	...
	{
		//如果是碰
		if(pDesk->sUserData.m_UserGCPData[pDesk->sGameData.T_HuPai.byUser][i].byType == ACTION_PENG)
		{
			//如果是 风牌 或者 箭牌 
		...
		else if(pDesk->sUserData.m_UserGCPData[pDesk->sGameData.T_HuPai.byUser][i].byType == ACTION_BU_GANG || pDesk->sUserData.m_UserGCPData[pDesk->sGameData.T_HuPai.byUser][i].byType == ACTION_MING_GANG)
		{//如果是补杠||明杠 
			//风牌||箭牌 +2
			if(pDesk->sUserData.m_UserGCPData[pDesk->sGameData.T_HuPai.byUser][i].byData[0] >= CMjEnum::MJ_TYPE_FD && pDesk->sUserData.m_UserGCPData[pDesk->sGameData.T_HuPai.byUser][i].byData[0] <= CMjEnum::MJ_TYPE_BAI)
			{
		...
		else if(pDesk->sUserData.m_UserGCPData[pDesk->sGameData.T_HuPai.byUser][i].byType == ACTION_AN_GANG)	
		{//暗杠
			//风牌||箭牌 +2
			if(pDesk->sUserData.m_UserGCPData[pDesk->sGameData.T_HuPai.byUser][i].byData[0] >= CMjEnum::MJ_TYPE_FD && pDesk->sUserData.m_UserGCPData[pDesk->sGameData.T_HuPai.byUser][i].byData[0] <= CMjEnum::MJ_TYPE_BAI)
			{
...
{    
	...
	char str[800];
	sprintf(str,"HZMJEX8:------99999--GF_NORMAL-------");
	OutputDebugString(str);
	//调用平台结束游戏接口
...
void	ThingHandleEx::HandlePengPai(BYTE &nextID,BYTE &time)	
{
	...
	pDesk->sGameData.T_PengPai.byPs = pDesk->sGameData.T_OutPai.byPs;		//被碰的牌数据
	...
	//吃完牌后检测动作
	if(CheckAction(pDesk->sUserData.m_byNowOutStation,peng.iOutpai/*pDesk->sUserData.GetLastHandPai(pDesk->sUserData.m_byNowOutStation)*/,5))
	{
		//标记是抓牌后有动作(客户端用于显示倒计时时间的)
		pDesk->sGameData.T_CPGNotify[pDesk->sUserData.m_byNowOutStation].bZhuaPai	 = true;
		pDesk->SendGameDataEx(pDesk->sGameData.m_byNtStation, &pDesk->sGameData.T_CPGNotify ,sizeof(pDesk->sGameData.T_CPGNotify), MDM_GM_GAME_NOTIFY, THING_CPG_NOTIFY, 0);
...
void	ThingHandleEx::HandleGangPai(BYTE &nextID,BYTE &time)	
{
	...
	if(pDesk->sGameData.m_byThingDoing == THING_OUT_PAI)
	{
		byGangType = ACTION_MING_GANG;
		//DebugPrintf("玩家%d明杠, 玩家%d被杠", pDesk->sGameData.T_GangPai.byUser, pDesk->sGameData.T_GangPai.byBeGang);
	}
	else if(pDesk->sGameData.m_byThingDoing == THING_ZHUA_PAI || THING_ONE_BUHUA == pDesk->sGameData.m_byThingDoing || THING_BEGIN_OUT == pDesk->sGameData.m_byThingDoing)	
	{
		...
		for(int i=0; i<5; i++)
		{
			...
			{
				byGangType = ACTION_BU_GANG;
				break;
			}
		}

		if(byGangType != ACTION_BU_GANG)
		{
			byGangType = ACTION_AN_GANG;
		}
	}
	...
	//如果是明杠
	if(byGangType == ACTION_MING_GANG)
	{
		//删除碰牌玩家的手上3张牌
		for(int i=0;i<3;i++)
		{	
		...
		pDesk->sUserData.m_TCalculateBoard[gang.iStation].iMingGan++;
	//	DebugPrintf("玩家%d的明杠次数=%d",gang.iStation, pDesk->sUserData.m_TCalculateBoard[gang.iStation].iMingGan);
	}
	else if(byGangType == ACTION_BU_GANG)	//补杠
	{
		//删除碰牌玩家的手上1张牌
		for(int i=0;i<1;i++)
		{	
	...
	else if(byGangType == ACTION_AN_GANG)	//暗杠
	{
		//删除碰牌玩家的手上4张牌
		for(int i=0;i<4;i++)
		{	
		...
		pDesk->sUserData.m_TCalculateBoard[gang.iStation].iAnGan++;
	//	DebugPrintf("玩家%d的暗杠次数=%d",gang.iStation, pDesk->sUserData.m_TCalculateBoard[gang.iStation].iAnGan);
	}

	//拷贝出牌数据
	memcpy(pDesk->sGameData.T_GangPai.m_byArOutPai, pDesk->sUserData.m_byArOutPai, sizeof(pDesk->sGameData.T_GangPai.m_byArOutPai));
	for (int i = 0; i < PLAY_COUNT; i++)
	{
		pDesk->sGameData.T_GangPai.m_byArOutPaiCount[i] = pDesk->sUserData.CopyOutPai(i, pDesk->sGameData.T_GangPai.m_byArOutPai[i]);
	}
...
//	pDesk->sUserData.m_bIsGangPao[pDesk->sGameData.temp_GangPai.byUser] = true;//正在杠后炮的状态
	...
	////如果是补杠就要检测是否有人抢杠胡 
	if(byGangType == ACTION_BU_GANG && CheckAction(gang.iStation,gang.iOutpai,3))
	{
		bool bHaveSend = false;	//是否已经发送了消息
		BYTE byCanActionDesk = 255;	//可以做动作的玩家
		//有人抢杠胡 那么就发送拦牌消息
...
void ThingHandleEx::SendTingPaiList(BYTE position)
{
	bool bHu = false;
	BYTE	byTingCan[MEN_CARD_NUM];//能胡的牌
	memset(byTingCan,255,sizeof(byTingCan));
	tagTingHuPaiMsg HuPaiMsg;
	HuPaiMsg.byUser = position;
	//发送听胡牌信息
	if(pDesk->sCheckHuPai.CanTingHu(position, pDesk->sGameData.T_CPGNotify[position].byHuType, byTingCan))
	{
		int n =0;
		for(int x=0; x<MEN_CARD_NUM-1;x++)
		{
			if(byTingCan[x] == 255) continue;
			HuPaiMsg.byHuPaiCard[x] = byTingCan[x];
			n++;
			bHu = true;
		}
		HuPaiMsg.byHuPaiCard[n] =35;
		memset(HuPaiMsg.byHuPaiCard+n+1, 255, MEN_CARD_NUM-n-1);
	}
	if (!bHu)
	{
		memset(HuPaiMsg.byHuPaiCard,255,sizeof(HuPaiMsg.byHuPaiCard));
	}
	...
	::memcpy(&pDesk->m_userTingPai[position], &HuPaiMsg, sizeof(pDesk->m_userTingPai[position]));
}
	...
	case THING_OUT_PAI://出牌事件
		{
			...
			{
				//OutputDebugString()
				SetOutTime(byStation,255,thing,true);
				if(!pDesk->IsBuyDesk())
				{
					if(255 == byTime)
					{
						if (pDesk->m_pUserInfo[byStation]->m_UserData.isVirtual)
						{
							SetOutTime(byStation, rand()%3+5, thing, true);
						}
						else
						{
							SetOutTime(byStation, pDesk->sGameData.m_mjRule.byOutTime, thing, true);
...
void	ThingHandleEx::OutTimeHandle()
{
	...
	if(pDesk->m_AutoActionWhenTooManyTimeout)
	{
		pDesk->sUserData.m_byNoOperateTime[pDesk->sGameData.m_MjWait.byCurChair]++;
		//DebugPrintf("玩家%d,%s %d秒无操作", pDesk->sGameData.m_MjWait.byCurChair, GetUserNickName(pDesk->sGameData.m_MjWait.byCurChair), pDesk->sUserData.m_byNoOperateTime[pDesk->sGameData.m_MjWait.byCurChair]);

		if (pDesk->sUserData.m_byNoOperateTime[pDesk->sGameData.m_MjWait.byCurChair] > pDesk->sGameData.m_mjRule.byTuoGuanTime)  //累计不操作的时间超过60秒则托管
		{

			if( !pDesk->sUserData.m_bTuoGuan[pDesk->sGameData.m_MjWait.byCurChair])
			{
				//DebugPrintf("玩家%d,%s 60秒无操作则托管", pDesk->sGameData.m_MjWait.byCurChair, GetUserNickName(pDesk->sGameData.m_MjWait.byCurChair));
				...
				pDesk->sUserData.m_byNoOperateTime[pDesk->sGameData.m_MjWait.byCurChair] = 0;
			}
		}
	}
...
{
	//pDesk->sUserData.m_bAuto[pDesk->sGameData.m_MjWait.byCurChair] = true;
...
void	ThingHandleEx::AutoOutOrGet()
{
	...
	for(int i=0;i<PLAY_COUNT;++i)
	{
		...
		// 不能打红中
		if(outpai.byPs == CMjEnum::MJ_TYPE_ZHONG)
		{
			BYTE hongZhong[] = {CMjEnum::MJ_TYPE_ZHONG};
			outpai.byPs = pDesk->sUserData.FindFirstHandCardNotIn(i, hongZhong, 1);
		};

		//DebugPrintf("玩家%d, 自动出牌%d", outpai.byUser, outpai.byPs);
...
void ThingHandleEx::SetGameStationBase(GameStation_Base *TGameStation, BYTE GameStation)
{
	TGameStation->iVersion		= MAIN_VERSION;
	TGameStation->iVersion2		= ASS_VERSION;
	TGameStation->byGameStation	= GameStation;
