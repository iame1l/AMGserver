...
bool CheckHuPaiEx::CanTing(BYTE station,BYTE byHuType[])
{
	...
	for(int i=1;i<30;++i)
	{
		...
		if(CanHu(station,i,byHuType,false))
		{
			canhu = true;
			break;
		}
...
///听胡检测
bool CheckHuPaiEx::CanTingHu(BYTE station,BYTE byHuType[],BYTE byHuCard[])
{
	if(station<0 || station>PLAY_COUNT)
	{
		return false;
	...
	for(int i=1;i<30;++i)
	{
		...
		if(CanHu(station,i,byHuType,false))
		{
			if(byHuCard[n] == 255 && n < MEN_CARD_NUM)
			{
				byHuCard[n] = i;
				n++;
			}
...
bool CheckHuPaiEx::CanHu(BYTE station,BYTE lastpai,BYTE hutype[],bool zimo)
{
	...
	{
		byArHandPai[j] = pDesk->sUserData.m_byArHandPai[station][j];
		//DebugPrintf("CanHu, 玩家%d的手牌%d=%d", station, j, byArHandPai[j]);
	}
	for(int i=0;i<HAND_CARD_NUM;i++)
	{
		if(byArHandPai[i] == 255 && lastpai != 255 && !zimo)
		{
			byArHandPai[i] = lastpai;
			break;
		}
	}
	...
	///从小到大排序
	MaoPaoSort(byArHandPai,HAND_CARD_NUM,false);
	
	bool bb = false;
	bool dahu = false;
	///牌数据转换
	ChangeHandPaiData(byArHandPai,255);	
	///统计根数
	int nGengCount = 0;
	nGengCount = CountGen();
	if( CheckPingHu(lastpai,zimo,false) || CheckFourZhong())//平糊组牌
	{        
...
bool CheckHuPaiEx::CheckPingHu(BYTE lastzhuapai,bool zimo,bool jing,bool CheckTing)
{
	...
	//最后手牌只有一张红中，又摸到一张红中的情况
	if(m_byJingNum == 2 && NoJingHuTempData.conut == 0)
		return true;
