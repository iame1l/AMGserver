...
UserDataEx::UserDataEx(void)
{
	...
	memset(m_byNoOperateTime, 0, sizeof(m_byNoOperateTime));
...
int UserDataEx::CheckCanPeng(BYTE station,BYTE pai)
{
	BYTE byTempArHandPai[14];
	memcpy(byTempArHandPai,m_byArHandPai[station],sizeof(byTempArHandPai));
	int count = 0;
	//检测碰了之后，是否能够出牌
	for(int i=0;i<HAND_CARD_NUM;i++)
	{
		//是碰的牌  || 或者红中 || 或者非法牌
		if(byTempArHandPai[i] == pai /*|| byTempArHandPai[i] == 35*/ || byTempArHandPai[i] == 255)continue;
		count++;
	}
	return count;
}
...
BYTE UserDataEx::FindFirstHandCardNotIn(BYTE station, BYTE cards[], int cnt) const
{
	BYTE ret = 255;
	for(int i = 0; i < m_byArHandPaiCount[station]; ++i)
	{
		bool exsits = false;
		for(int j = 0; j < cnt; ++j)
		{
			if(m_byArHandPai[station][i] == cards[j])
			{
				exsits = true;
				break;
			}
