...
struct tagSuperRemainingPai
{
	BYTE	byPaiCount;					// 牌数量
	BYTE	byPai[TOTAL_CARD_COUNT];	// 牌库

	tagSuperRemainingPai():byPaiCount(0){}
};
...
struct tagCountFenEx:tagCountFen
{
	...
	BYTE	byRemaindCard[REMAIND_CARD_COUNT];			//剩余未显示的牌
	int		iRemaindCardCount;					//剩余牌数
	BYTE	byZamaCard[ZAMA_COUNT];				//扎马的牌数据
	BYTE	byHuPs;								//胡的牌
	GCPStructEx	tagUserGCPData[PLAY_COUNT][5];	//吃碰杠的牌
	...
	void Clear()
	{
		...
		iBase = 0;
		iHuangZhuangBase = 1;
		iRemaindCardCount = 112;
		memset(byRemaindCard, 255, sizeof(byRemaindCard));
		memset(byZamaCard, 255, sizeof(byZamaCard));
...
struct GameStationDataEx:GameStationData
{
	...
	tagCPGNotifyEx		m_CPGNotify[PLAY_COUNT];				//吃碰杠糊牌通知事件牌事件
	BYTE				m_byArOutPai[PLAY_COUNT][OUT_CARD_NUM];	//出牌数据
	BYTE                m_byNowOutStation;
	BYTE                m_byNowThing;
	int					m_iRemaindCardCount;			//剩余牌数
