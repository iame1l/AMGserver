...
bool CheckCPGActionEx::CanPeng(BYTE station,BYTE pai)
{
	...
	{
		return pDesk->sUserData.CheckCanPeng(station,pai);
	}
	return false;
}
...
bool CheckCPGActionEx::CanGang(BYTE station,BYTE pai,BYTE Result[][2],bool me)
{
	...
	{	
		...
		for(int i=0;i<HAND_CARD_NUM;i++)
		{
			...
			if(pDesk->sUserData.GetAHandPaiCount(station,pDesk->sUserData.m_byArHandPai[station][i])>=4 && temp != pDesk->sUserData.m_byArHandPai[station][i])
			{
				...
				//DebugPrintf("CheckActon, 玩家%d暗杠：pai=%d", station, pDesk->sUserData.m_byArHandPai[station][i]);
				bb = true;
				index++;
				continue;
			}
		...
		{
			if(pDesk->sUserData.IsUserHavePengPai(station,/*pDesk->sUserData.m_byArHandPai[station][i]*/ pai))  //参数改为pai,为了解决补杠pass后一直提示杠
			{
				Result[index][1] = pai;
				Result[index][0] = ACTION_BU_GANG;
				temp = pai;
				//DebugPrintf("CheckActon, 玩家%d补杠：pai=%d", station, pai);
				bb = true;
				// index++;
			}
		}
	...
	else//明杠
	{
		...
		if(pDesk->sUserData.GetAHandPaiCount(station,pai)>=3 )
		{
			...
			//DebugPrintf("CheckActon, 玩家%d明杠：pai=%d", station, pai);
		}
	}
	return bb;
