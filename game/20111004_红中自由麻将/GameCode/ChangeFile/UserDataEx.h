...
public:	
	...
	BYTE m_byNoOperateTime[PLAY_COUNT];			//玩家不操作时间
	bool m_bAuto[PLAY_COUNT];					//玩家是否自动操作
...
public:
	...
	int CheckCanPeng(BYTE station,BYTE pai);
	...
	// 找到第一张不在cards中的手牌
	BYTE FindFirstHandCardNotIn(BYTE station, BYTE cards[], int cnt) const;
