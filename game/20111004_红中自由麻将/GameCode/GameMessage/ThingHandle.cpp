...
bool ThingHandle::GetGameStaton(BYTE bDeskStation, bool bWatchUser,BYTE GameStation)
{
	...
	//GameStationData.Stationpara = pDesk->m_byGameStation;		 //游戏状态
	////断线重连取消托管
	...
	//GameStationData.byThinkTime = pDesk->sGameData.m_mjRule.byBlockTime;			//思考时间
...
int ThingHandle::ReceiveUserTuoGuan(BYTE bDeskStation, void * pData, UINT uSize, bool bWatchUser)
{
	if(bWatchUser)
		return 0;
	if(bWatchUser || pDesk->m_byGameStation > GS_PLAY_GAME || pDesk->m_byGameStation < GS_SEND_CARD)//不在游戏中不能托管
		return 0;
...
//	pDesk->sGameData.T_HuPai.byNt = pDesk->sGameData.m_byNtStation;		//庄家位置
	...
	{//点炮糊牌，并且有多个玩家可以糊牌
		pDesk->sGameData.T_HuPai.byUser = GetCanHuUser(pDesk->sUserData.m_byNowOutStation,pDesk->sGameData.T_HuPai.bHaveHu,pDesk->sGameData.T_HuPai.byUser);//获取离糊牌玩家近的玩家
	}
...
void ThingHandle::ExecuteGameBegin(BYTE &nextID,BYTE &time)
{
	...
	else 
	{
		...
		nextID = THING_DING_NT;//执行以东为庄事件
	}
	pDesk->sGameData.T_Begin.byNt = pDesk->sGameData.m_byNtStation;			// 庄家位置				
...
void ThingHandle::ExecuteTwoSeziNt(BYTE &nextID,BYTE &time)	
{
	...
	nextID = THING_2SEZI_DIR;//THING_2SEZI_DIR_GP;
	time = 2;	//200毫秒间隔
	///掷2颗色子的点数和为庄事件
...
//	pDesk->sGameData.T_BeginOutPai.m_StrJing = pDesk->sUserData.m_StrJing;//精牌
	...
	//if(pDesk->sUserData.m_bFoceLeave[pDesk->sGameData.m_byNtStation] && pDesk->sGameData.m_mjRule.bNetCutTuoGuan)//如果庄家断线,而且有断线托管
	//{
...
//	pDesk->sGameData.T_CountFen.byLianZhuang = pDesk->sUserData.m_byLianZhuang;
	...
	{///庄赢连庄，否则下家坐庄
//		if(!pDesk->sGameData.T_CountFen.bHu[pDesk->sGameData.m_byNtStation] || pDesk->sGameData.T_CountFen.byUser==255)//糊牌中没有庄家或者流局
		{
...
BYTE ThingHandle::GetPai(int station,bool head,BYTE type,BYTE *pIndex)
{
	...
	if(pDesk->m_pUserInfo[station])
	{
		...
		card = GetSpecifiedCard(pDesk->m_pUserInfo[station]->m_UserData.dwUserID, card, 
			...
			pDesk->sUserData.GetHandPaiCount(station) == 0,
			pDesk->m_SuperSetCard);
	}
...
BYTE ThingHandle::GetCard(bool head, BYTE *pIndex)
{
	BYTE paiIndex = 255;//被抓牌的位置
	int num=0;//遍历次数
	BYTE card=255;
