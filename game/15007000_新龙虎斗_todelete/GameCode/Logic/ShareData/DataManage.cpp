...
bool DataManage::shuffleCard()
{
	...
	//20190415 eil 先生成比牌让看起来正常一点
	{

		int iLong = rand() % 13 + 1;
		int iHu = rand() % 13 + 1;
	
		CardBase::GetRandOneCard(iLong, vSendCard[0]);
		CardBase::GetRandOneCard(iHu, vSendCard[1]);
	}
...
void DataManage::GetAIContrlSetFromIni()
{
	...
	m_pingjuPro = f.GetKeyVal(szSec, "pingjupro", 50);     //平局概念
}
